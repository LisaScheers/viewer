/**
 * @file llwindowwin32vulkan_test.cpp
 * @brief Tests for isolated Win32 Vulkan window and loader ownership.
 *
...
struct FakeState
{
    ...
    void* mModuleInstance = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x3030));
    void* mWindow         = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x4040));

    U64 mOwnerThreadId   = 77;
    U64 mCurrentThreadId = 77;

    LLWindowWin32VulkanNativeWindow                    mNative{ mToken, mModuleInstance, mWindow, mOwnerThreadId, 1280, 720 };
    std::optional<LLWindowWin32VulkanNativeCreateCode> mNativeFailure;
    ...
    const wchar_t*                       mOpenedPath     = nullptr;
    void*                                mResolverLoader = nullptr;
    const LLWindowWin32VulkanCreateInfo* mCreateInfo     = nullptr;
    LLWindowWin32VulkanNativeWindow      mDestroyedNative;
    ...
    void*                        mSurfaceModuleInstance = nullptr;
    void*                        mSurfaceWindow         = nullptr;
    ...
    const LLWindowWin32Vulkan* mOwnerDuringDestroy                     = nullptr;
                                                                        ...
                                                                        VkExtensionProperties* properties) noexcept
{
    ...
    constexpr std::array<const char*, 3> extensions{ VK_KHR_SURFACE_EXTENSION_NAME, "VK_KHR_win32_surface",
...
U64 currentThreadId(void* userdata) noexcept
{
    return static_cast<FakeState*>(userdata)->mCurrentThreadId;
}

void* openLoader(void* userdata, const wchar_t* path) noexcept
{
...
LLWindowWin32VulkanNativeCreateResult createNativeWindow(void* userdata, const LLWindowWin32VulkanCreateInfo& info) noexcept
{
    auto& state = *static_cast<FakeState*>(userdata);
    ...
    {
        return LLWindowWin32VulkanNativeCreateError{ *state.mNativeFailure };
    }
    state.mNativeLive = true;
    return state.mNative;
}

bool refreshNativeWindow(void* userdata, LLWindowWin32VulkanNativeWindow& native) noexcept
{
    auto& state = *static_cast<FakeState*>(userdata);
    ...
    native.mClientWidth  = state.mRefreshWidth;
    native.mClientHeight = state.mRefreshHeight;
    switch (state.mRefreshMutation)
    {
        ...
        case RefreshMutation::Token:
            ...
            break;
        case RefreshMutation::ModuleInstance:
            native.mModuleInstance = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x9090));
            break;
        case RefreshMutation::Window:
            native.mWindow = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x9090));
            break;
        case RefreshMutation::OwnerThread:
            native.mOwnerThreadId = 88;
            break;
        case RefreshMutation::ZeroWidth:
            native.mClientWidth = 0;
            break;
        case RefreshMutation::ZeroHeight:
            native.mClientHeight = 0;
            break;
    }
    return true;
}

void destroyNativeWindow(void* userdata, LLWindowWin32VulkanNativeWindow& native) noexcept
{
    auto& state = *static_cast<FakeState*>(userdata);
                                                         ...
                                                         VkSurfaceKHR*                surface) noexcept
{
    ...
    state.mSurfaceModuleInstance = module_instance;
...
LLWindowWin32VulkanOperations fakeOperations(FakeState& state) noexcept
{
    return { &state,       currentThreadId,    openLoader,          closeLoader,
             getResolver,  createNativeWindow, refreshNativeWindow, destroyNativeWindow,
             createSurface };
}

LLWindowWin32VulkanCreateInfo createInfo()
{
    return { L"C:\\diagnostic\\vulkan-1.dll", 77, 1280, 720 };
}
...
const LLWindowWin32VulkanAcquireError* acquireError(const LLWindowWin32VulkanAcquireResult& result) noexcept
{
    return std::get_if<LLWindowWin32VulkanAcquireError>(&result);
}

LLWindowWin32Vulkan* acquiredWindow(LLWindowWin32VulkanAcquireResult& result) noexcept
{
    return std::get_if<LLWindowWin32Vulkan>(&result);
}

void ensureAcquireError(const char* message, const LLWindowWin32VulkanAcquireResult& result, LLWindowWin32VulkanAcquireCode code)
{
...
struct window_win32_vulkan_test
{
};

using window_win32_vulkan_group  = test_group<window_win32_vulkan_test>;
using window_win32_vulkan_object = window_win32_vulkan_group::object;
window_win32_vulkan_group window_win32_vulkan_tests("window Win32 Vulkan ownership");

template<>
template<>
void window_win32_vulkan_object::test<1>()
{
    static_assert(!std::is_copy_constructible_v<LLWindowWin32Vulkan>);
    static_assert(!std::is_copy_assignable_v<LLWindowWin32Vulkan>);
    static_assert(std::is_nothrow_move_constructible_v<LLWindowWin32Vulkan>);
    static_assert(std::is_nothrow_move_assignable_v<LLWindowWin32Vulkan>);
    static_assert(std::is_nothrow_destructible_v<LLWindowWin32Vulkan>);
    static_assert(std::is_same_v<decltype(std::declval<const LLWindowWin32Vulkan&>().requirements()), const LLWindowVulkanRequirements*>);
    static_assert(noexcept(acquireLLWindowWin32Vulkan(std::declval<const LLWindowWin32VulkanCreateInfo&>(),
                                                      U64{},
                                                      std::declval<const LLWindowWin32VulkanOperations&>())));

    const auto& operations = defaultLLWindowWin32VulkanOperations();
    ensure("the default native operation table is complete",
           operations.mCurrentThreadId && operations.mOpenLoader && operations.mCloseLoader && operations.mGetResolver &&
...
template<>
template<>
void window_win32_vulkan_object::test<2>()
{
    FakeState   state;
    ScopedState active(state);
    auto        info   = createInfo();
    auto        result = acquireLLWindowWin32Vulkan(info, 41, fakeOperations(state));
    ...
    ensure("the description preserves loader and client dimensions",
           state.mCreateInfo->mLoaderPath == L"C:\\diagnostic\\vulkan-1.dll" && state.mCreateInfo->mOwnerThreadId == state.mOwnerThreadId &&
               state.mCreateInfo->mClientWidth == 1280 && state.mCreateInfo->mClientHeight == 720 &&
               std::wcscmp(state.mOpenedPath, state.mCreateInfo->mLoaderPath.c_str()) == 0);
    ensure("the owner publishes only scalar native state",
           owner->hasNativeWindow() && owner->ownerThreadId() == state.mOwnerThreadId && owner->clientWidth() == 1280 &&
               owner->clientHeight() == 720);
    ...
    const auto& extensions = owner->requirements()->requiredInstanceExtensions();
           ...
           extensions == std::vector<std::string>{ VK_KHR_SURFACE_EXTENSION_NAME, "VK_KHR_win32_surface" } &&
    ...
    ensure("refreshed geometry is published", owner->clientWidth() == 1440 && owner->clientHeight() == 900);

    ensure("the complete Vulkan ownership chain is acquired before the affinity check",
           !owner->acquireInstanceGeneration(LLRenderVulkan::VulkanInstanceValidationMode::Required) && !owner->acquireSurfaceGeneration());
    ...
    state.mCurrentThreadId = 88;
    ensure("off-thread surface reset fails without releasing its child", !owner->resetSurfaceGeneration());
    ensure("off-thread reset fails without releasing any owner", !owner->reset());
    ensure("off-thread reset preserves the complete ownership chain",
    ...
    ensure("off-thread reset destroys no Vulkan owner",
           state.mDestroySurfaceCount == 0 && state.mDestroyDebugCount == 0 && state.mDestroyInstanceCount == 0);
    ensure_equals("off-thread reset invokes no teardown operation", state.mEventCount, std::size_t{ 9 });

    state.mCurrentThreadId    = state.mOwnerThreadId;
    state.mOwnerDuringDestroy = owner;
    ensure("owner-thread reset releases the complete owner", owner->reset());
    ...
    ensure("native destruction receives the exact token, module, window, and thread",
           state.mDestroyedNative.mToken == state.mToken && state.mDestroyedNative.mModuleInstance == state.mModuleInstance &&
               state.mDestroyedNative.mWindow == state.mWindow && state.mDestroyedNative.mOwnerThreadId == state.mOwnerThreadId);
...
template<>
template<>
void window_win32_vulkan_object::test<3>()
{
    ...
    auto invalid           = acquireLLWindowWin32Vulkan(info, 1, operations);
    ensureAcquireError("an incomplete operation table is rejected", invalid, LLWindowWin32VulkanAcquireCode::InvalidOperations);
    ...
    operations.mCurrentThreadId = nullptr;
    invalid                     = acquireLLWindowWin32Vulkan(info, 1, operations);
    ensureAcquireError("a missing owner-thread operation is rejected", invalid, LLWindowWin32VulkanAcquireCode::InvalidOperations);

    operations                      = fakeOperations(invalid_state);
    operations.mRefreshNativeWindow = nullptr;
    invalid                         = acquireLLWindowWin32Vulkan(info, 1, operations);
    ensureAcquireError("a missing refresh operation is rejected", invalid, LLWindowWin32VulkanAcquireCode::InvalidOperations);

    operations                = fakeOperations(invalid_state);
    operations.mCreateSurface = nullptr;
    invalid                   = acquireLLWindowWin32Vulkan(info, 1, operations);
    ensureAcquireError("a missing surface operation is rejected", invalid, LLWindowWin32VulkanAcquireCode::InvalidOperations);

    auto invalid_thread           = info;
    invalid_thread.mOwnerThreadId = 0;
    auto thread_id                = acquireLLWindowWin32Vulkan(invalid_thread, 1, fakeOperations(invalid_state));
    ensureAcquireError("a zero owner thread is rejected", thread_id, LLWindowWin32VulkanAcquireCode::InvalidCreateInfo);

    auto invalid_width         = info;
    invalid_width.mClientWidth = 0;
    auto width                 = acquireLLWindowWin32Vulkan(invalid_width, 1, fakeOperations(invalid_state));
    ensureAcquireError("zero client width is rejected", width, LLWindowWin32VulkanAcquireCode::InvalidCreateInfo);

    auto invalid_height          = info;
    invalid_height.mClientHeight = 0;
    auto height                  = acquireLLWindowWin32Vulkan(invalid_height, 1, fakeOperations(invalid_state));
    ensureAcquireError("zero client height is rejected", height, LLWindowWin32VulkanAcquireCode::InvalidCreateInfo);

    auto oversized_width         = info;
    oversized_width.mClientWidth = static_cast<U32>(std::numeric_limits<int>::max()) + U32{ 1 };
    auto oversized               = acquireLLWindowWin32Vulkan(oversized_width, 1, fakeOperations(invalid_state));
    ensureAcquireError("a client extent outside Win32 int range is rejected", oversized, LLWindowWin32VulkanAcquireCode::InvalidCreateInfo);
    ...
    thread_state.mCurrentThreadId = 88;
    auto thread                   = acquireLLWindowWin32Vulkan(info, 1, fakeOperations(thread_state));
    ensureAcquireError("off-thread acquisition is rejected", thread, LLWindowWin32VulkanAcquireCode::OwnerThreadRequired);
    ensure_equals("off-thread acquisition has no process or loader side effect", thread_state.mEventCount, std::size_t{ 0 });

    FakeState loader_state;
    loader_state.mLoaderOpens = false;
    auto loader               = acquireLLWindowWin32Vulkan(info, 1, fakeOperations(loader_state));
    ensureAcquireError("loader failure is typed", loader, LLWindowWin32VulkanAcquireCode::LoaderFailure);
    ...
    auto resolver                = acquireLLWindowWin32Vulkan(info, 1, fakeOperations(resolver_state));
    ensureAcquireError("resolver failure is typed", resolver, LLWindowWin32VulkanAcquireCode::ResolverFailure);
...
template<>
template<>
void window_win32_vulkan_object::test<4>()
{
    constexpr std::array failures{
        LLWindowWin32VulkanNativeCreateCode::InvalidRequest, LLWindowWin32VulkanNativeCreateCode::StorageFailure,
        LLWindowWin32VulkanNativeCreateCode::ModuleFailure,  LLWindowWin32VulkanNativeCreateCode::ClassRegistrationFailure,
        LLWindowWin32VulkanNativeCreateCode::WindowFailure,  LLWindowWin32VulkanNativeCreateCode::IdentityFailure,
    ...
    for (const auto code : failures)
    {
        ...
        auto result          = acquireLLWindowWin32Vulkan(createInfo(), 1, fakeOperations(state));
        ensureAcquireError("native creation failure is typed", result, LLWindowWin32VulkanAcquireCode::NativeWindowFailure);
        const auto* error = acquireError(result);
...
template<>
template<>
void window_win32_vulkan_object::test<5>()
{
    const auto info = createInfo();
    ...
    for (std::size_t identity = 0; identity < 4; ++identity)
    {
        ...
        {
            state.mNative.mModuleInstance = nullptr;
        }
        else if (identity == 2)
        {
        ...
        {
            state.mNative.mOwnerThreadId = 0;
        }
        auto result = acquireLLWindowWin32Vulkan(info, 1, fakeOperations(state));
        ensureAcquireError("a poisoned native identity is rejected", result, LLWindowWin32VulkanAcquireCode::NativeWindowIdentityFailure);
    ...
    for (std::size_t geometry = 0; geometry < 3; ++geometry)
    {
        FakeState state;
        switch (geometry)
        {
            case 0:
                state.mNative.mClientWidth = 0;
                break;
            case 1:
                state.mNative.mClientHeight = 0;
                break;
            default:
                state.mNative.mClientWidth = info.mClientWidth + 1;
                break;
        }
        auto result = acquireLLWindowWin32Vulkan(info, 1, fakeOperations(state));
        ensureAcquireError("invalid or mismatched native geometry is rejected", result,
                           LLWindowWin32VulkanAcquireCode::NativeWindowGeometryFailure);
...
template<>
template<>
void window_win32_vulkan_object::test<6>()
{
    const auto info = createInfo();

    FakeState generation_state;
    auto      generation = acquireLLWindowWin32Vulkan(info, 0, fakeOperations(generation_state));
    ensureAcquireError("zero native generation is rejected before acquisition", generation,
                       LLWindowWin32VulkanAcquireCode::InvalidNativeWindowGeneration);
    ...
    ensure_equals("invalid generation creates no loader, Win32 state, or native window", generation_state.mEventCount, std::size_t{ 0 });

    FakeState allocation_state;
    auto      allocation = LLWindowWin32VulkanDetail::acquire(info, 1, fakeOperations(allocation_state), failAllocation);
    ensureAcquireError("requirements allocation failure is typed", allocation, LLWindowWin32VulkanAcquireCode::RequirementsFailure);
...
template<>
template<>
void window_win32_vulkan_object::test<7>()
{
    using namespace LLRenderVulkan;

    FakeState   state;
    ScopedState active(state);
    auto        result = acquireLLWindowWin32Vulkan(createInfo(), 17, fakeOperations(state));
    auto*       owner  = acquiredWindow(result);
    ...
    ensure("failed refresh preserves published geometry", owner->clientWidth() == 1280 && owner->clientHeight() == 720);

    state.mRefreshSucceeds = true;
    constexpr std::array identity_mutations{ RefreshMutation::Token, RefreshMutation::ModuleInstance, RefreshMutation::Window,
                                             RefreshMutation::OwnerThread };
    ...
    constexpr std::array geometry_mutations{ RefreshMutation::ZeroWidth, RefreshMutation::ZeroHeight };
    for (const auto mutation : geometry_mutations)
    {
        state.mRefreshMutation = mutation;
        ensure("poisoned refresh geometry is rejected", !owner->refreshNativeGeometry());
        ensure("poisoned geometry does not replace the last valid snapshot", owner->clientWidth() == 1280 && owner->clientHeight() == 720);
    }

    state.mRefreshMutation    = RefreshMutation::OwnerThread;
    const auto stale_instance = owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Disabled);
    ...
    ensure("recovery publishes the new scalar geometry", owner->clientWidth() == 1600 && owner->clientHeight() == 1000);
    ...
    ensure("recovery retains the original module and window",
           state.mSurfaceModuleInstance == state.mModuleInstance && state.mSurfaceWindow == state.mWindow &&
               state.mSurfaceResolver == fakeResolver());
...
template<>
template<>
void window_win32_vulkan_object::test<8>()
{
    using namespace LLRenderVulkan;

    FakeState   state;
    ScopedState active(state);
    auto        result = acquireLLWindowWin32Vulkan(createInfo(), 31, fakeOperations(state));
    auto*       owner  = acquiredWindow(result);
    ensure("lifecycle fixture acquired native state", owner != nullptr);

    const auto instance_error = owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Required);
    ensure("a validated non-portable Vulkan 1.1 instance is acquired", !instance_error);
    ...
    ensure("the Win32 surface generation is acquired", !owner->acquireSurfaceGeneration());
    ...
    ensure("surface creation receives exact resolver, module, window, instance, and allocator identities",
           state.mSurfaceResolver == fakeResolver() && state.mSurfaceModuleInstance == state.mModuleInstance &&
               state.mSurfaceWindow == state.mWindow && state.mSurfaceInstance == state.mInstance && !state.mSurfaceAllocator);
    ...
    const auto duplicate_instance = owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Required);
    ...
    LLWindowWin32Vulkan moved(std::move(*owner));
...
template<>
template<>
void window_win32_vulkan_object::test<9>()
{
    using namespace LLRenderVulkan;

    FakeState   state;
    ScopedState active(state);
    auto        result = acquireLLWindowWin32Vulkan(createInfo(), 51, fakeOperations(state));
    auto*       owner  = acquiredWindow(result);
    ...
    ensure("surface failure fixture acquired an instance", !owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Disabled));
...
template<>
template<>
void window_win32_vulkan_object::test<10>()
{
    using namespace LLRenderVulkan;

    FakeState   state;
    ScopedState active(state);
    auto        result = acquireLLWindowWin32Vulkan(createInfo(), 61, fakeOperations(state));
    auto*       owner  = acquiredWindow(result);
    ensure("surface reset fixture acquired native state", owner != nullptr);
    ensure("surface reset fixture acquired an instance", !owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Disabled));

    const auto allocation_failure = LLWindowWin32VulkanDetail::acquireSurfaceGeneration(*owner, failAllocation);
...
template<>
template<>
void window_win32_vulkan_object::test<11>()
{
    using namespace LLRenderVulkan;
    ...
    source_state.mModuleInstance = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x13330));
    source_state.mWindow         = reinterpret_cast<void*>(static_cast<std::uintptr_t>(0x14440));
    source_state.mNative         = {
        source_state.mToken, source_state.mModuleInstance, source_state.mWindow, source_state.mOwnerThreadId, 1280, 720
    };
    ...
    auto        source_result = acquireLLWindowWin32Vulkan(createInfo(), 71, fakeOperations(source_state));
    auto*       source        = acquiredWindow(source_result);
    ensure("move source acquired native state", source != nullptr);
    ensure("move source acquired a validated instance", !source->acquireInstanceGeneration(VulkanInstanceValidationMode::Required));
    ensure("move source acquired a surface", !source->acquireSurfaceGeneration());

    active.use(destination_state);
    auto  destination_result = acquireLLWindowWin32Vulkan(createInfo(), 72, fakeOperations(destination_state));
    auto* destination        = acquiredWindow(destination_result);
    ensure("move destination acquired native state", destination != nullptr);
    ensure("move destination acquired a validated instance",
           !destination->acquireInstanceGeneration(VulkanInstanceValidationMode::Required));
...
template<>
template<>
void window_win32_vulkan_object::test<12>()
{
    using namespace LLRenderVulkan;

    FakeState   state;
    ScopedState active(state);
    auto        result = acquireLLWindowWin32Vulkan(createInfo(), 81, fakeOperations(state));
    auto*       owner  = acquiredWindow(result);
    ...
    const auto failure    = owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Disabled);
    ...
    ensure("instance acquisition can retry after failure", !owner->acquireInstanceGeneration(VulkanInstanceValidationMode::Disabled));
    state.mOwnerDuringDestroy = owner;
