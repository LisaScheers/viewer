...
enum class Event : std::uint8_t
{
    ...
    DestroyTextureImage,
    FreeTextureMemory,
    DestroyTextureUploadSourceBuffer,
    FreeTextureUploadSourceMemory,
    CreateTextureSampleSampler,
    CreateTextureSampleDescriptorSetLayout,
    CreateTextureSamplePipelineLayout,
    CreateTextureSampleDescriptorPool,
    AllocateTextureSampleDescriptorSet,
    UpdateTextureSampleDescriptorSet,
...
enum class BufferMemoryKind : std::uint8_t
{
    ...
    UploadDestination,
    TextureUploadSource,
    TextureUploadDestination
};

struct TextureUploadSourceTeardownObservation
{
    bool                                                    mSourceDetached                 = false;
    bool                                                    mSwapchainConfigurationDetached = false;
    bool                                                    mDestinationLive                = false;
    bool                                                        mLogicalDeviceLive              = false;
    bool                                                        mSurfaceLive                    = false;
    std::optional<VulkanTextureUploadSourceAcquireError>        mAcquireError;
    ...
    bool                                                        mConfigurationNativeWorkUnchanged = false;
    bool                                                    mMoveProducedEmpty                = false;
    bool                                                    mSourceResetSucceeded             = false;
    bool                                                    mDestinationResetSucceeded        = false;
    bool                                                    mLogicalResetSucceeded            = false;
    bool                                                    mSurfaceResetSucceeded            = false;
    bool                                                    mFullResetSucceeded               = false;
};

struct TextureUploadDestinationTeardownObservation
{
    bool                                                      mDestinationDetached       = false;
    bool                                                      mMoveProducedEmpty         = false;
    bool                                                      mDestinationResetSucceeded = false;
    bool                                                        mLogicalResetSucceeded     = false;
    bool                                                        mFullResetSucceeded        = false;
    ...
    std::optional<VulkanSwapchainConfigurationAcquireError>     mConfigurationAcquireError;
    std::optional<VulkanSwapchainChainRebuildError>             mRebuildError;
    std::optional<VulkanUploadTransferParentOperationError>     mLegacyOperationError;
    bool                                                      mLegacyOperationUnchanged = false;
};

struct TextureUploadTransferTeardownObservation
{
    bool                                                           mTransferDetached          = false;
    bool                                                           mMoveProducedEmpty         = false;
    bool                                                           mTransferResetSucceeded    = false;
    bool                                                           mSourceResetSucceeded      = false;
    bool                                                           mDestinationResetSucceeded = false;
    bool                                                           mLogicalResetSucceeded     = false;
    bool                                                           mFullResetSucceeded        = false;
    bool                                                           mNoAuthOrNativeWork        = false;
    std::optional<VulkanTextureUploadTransferAcquireError>         mAcquireError;
    std::optional<VulkanTextureUploadSampleBindingAcquireError>    mBindingAcquireError;
    std::optional<VulkanSwapchainConfigurationAcquireError>        mConfigurationAcquireError;
    std::optional<VulkanTextureUploadTransferParentOperationError> mOperationError;
    std::optional<VulkanSwapchainChainRebuildError>                mRebuildError;
};

struct TextureUploadSampleBindingTeardownObservation
{
    bool                                                        mBindingDetached           = false;
    bool                                                        mDestinationLive           = false;
    bool                                                        mSourceLive                = false;
    bool                                                        mTransferLive              = false;
    bool                                                        mMoveProducedEmpty         = false;
    ...
    std::optional<VulkanTextureUploadSampleBindingAcquireError> mBindingAcquireError;
    std::optional<VulkanTextureUploadDestinationAcquireError>   mDestinationAcquireError;
    std::optional<VulkanTextureUploadSourceAcquireError>        mSourceAcquireError;
...
struct FakeState
{
    ...
    VkFormatProperties              mTextureFormatProperties{};
    VkImageFormatProperties         mTextureImageFormatProperties{};
    VkResult                        mTextureImageFormatResult = VK_SUCCESS;
    ...
    VkImage                        mTextureUploadDestinationImage    = fakeHandle<VkImage>(0xf001);
    VkDeviceMemory                 mTextureUploadDestinationMemory   = fakeHandle<VkDeviceMemory>(0xf002);
    VkImageView                    mTextureUploadDestinationView     = fakeHandle<VkImageView>(0xf003);
    VkBuffer                       mTextureUploadSourceBuffer        = fakeHandle<VkBuffer>(0xf101);
    VkDeviceMemory                 mTextureUploadSourceMemory        = fakeHandle<VkDeviceMemory>(0xf102);
    VkSampler                      mTextureSampleSampler             = fakeHandle<VkSampler>(0xf201);
    VkDescriptorSetLayout          mTextureSampleDescriptorSetLayout = fakeHandle<VkDescriptorSetLayout>(0xf202);
    ...
    VulkanTextureUploadSourceBytes mTextureUploadSourceMappedStorage{};
    ...
    bool                            mObservedTextureUploadDestinationAtSurfaceDestroy   = false;
    bool                            mObservedTextureUploadSourceAtSurfaceDestroy        = false;
    bool                            mObservedTextureUploadSampleBindingAtSurfaceDestroy = false;
    ...
    bool                            mObservedTextureUploadDestinationAtDeviceDestroy   = false;
    bool                            mObservedTextureUploadSourceAtDeviceDestroy        = false;
    bool                            mObservedTextureUploadSampleBindingAtDeviceDestroy = false;
    ...
    std::size_t                     mTextureFormatPropertiesCalls                    = 0;
    std::size_t                     mTextureImageFormatPropertiesCalls               = 0;
    VkPhysicalDevice                mTextureFormatPropertiesDevice                   = VK_NULL_HANDLE;
    VkFormat                        mTextureFormatPropertiesFormat                   = VK_FORMAT_UNDEFINED;
    VkPhysicalDevice                mTextureImageFormatPropertiesDevice              = VK_NULL_HANDLE;
    VkFormat                        mTextureImageFormat                              = VK_FORMAT_UNDEFINED;
    VkImageType                     mTextureImageType                                = VK_IMAGE_TYPE_MAX_ENUM;
    VkImageTiling                   mTextureImageTiling                              = VK_IMAGE_TILING_MAX_ENUM;
    VkImageUsageFlags               mTextureImageUsage                               = 0;
    VkImageCreateFlags              mTextureImageFlags                               = 0;
    ...
    VkResult                                  mTextureImageCreateResult     = VK_SUCCESS;
    VkResult                                  mTextureMemoryAllocateResult  = VK_SUCCESS;
    VkResult                                  mTextureImageBindResult       = VK_SUCCESS;
    VkResult                                  mTextureImageViewCreateResult = VK_SUCCESS;
    bool                                      mNullTextureImage             = false;
    bool                                      mNullTextureMemory            = false;
    bool                                      mNullTextureImageView         = false;
    VkMemoryRequirements                      mTextureImageMemoryRequirements{};
    VkBool32                                  mTexturePrefersDedicatedAllocation      = VK_TRUE;
    VkBool32                                  mTextureRequiresDedicatedAllocation     = VK_TRUE;
    std::size_t                               mCreateTextureImageCalls                = 0;
    std::size_t                               mDestroyTextureImageCalls               = 0;
    std::size_t                               mGetTextureImageMemoryRequirementsCalls = 0;
    std::size_t                               mAllocateTextureMemoryCalls             = 0;
    std::size_t                               mFreeTextureMemoryCalls                 = 0;
    std::size_t                               mBindTextureImageMemoryCalls            = 0;
    ...
    VkImage                                   mTextureMemoryRequirementsImage = VK_NULL_HANDLE;
    VkMemoryAllocateInfo                      mTextureMemoryAllocateInfo{};
    VkMemoryDedicatedAllocateInfo             mTextureDedicatedAllocateInfo{};
    VkImage                                   mTextureBoundImage  = VK_NULL_HANDLE;
    VkDeviceMemory                            mTextureBoundMemory = VK_NULL_HANDLE;
    VkDeviceSize                              mTextureBindOffset  = std::numeric_limits<VkDeviceSize>::max();
    VkImageViewCreateInfo                     mTextureImageViewCreateInfo{};
    std::vector<VkImage>                      mCreatedTextureImages;
    std::vector<VkImage>                      mDestroyedTextureImages;
    std::vector<VkDeviceMemory>               mAllocatedTextureMemories;
    std::vector<VkDeviceMemory>               mFreedTextureMemories;
    std::vector<VkImageView>                  mCreatedTextureImageViews;
    std::vector<VkImageView>                  mDestroyedTextureImageViews;
    std::vector<const VkAllocationCallbacks*> mCreateTextureImageAllocationCallbacks;
    std::vector<const VkAllocationCallbacks*> mDestroyTextureImageAllocationCallbacks;
    std::vector<const VkAllocationCallbacks*> mAllocateTextureMemoryAllocationCallbacks;
    std::vector<const VkAllocationCallbacks*> mFreeTextureMemoryAllocationCallbacks;
    std::vector<const VkAllocationCallbacks*> mCreateTextureImageViewAllocationCallbacks;
    std::vector<const VkAllocationCallbacks*> mDestroyTextureImageViewAllocationCallbacks;

    VkResult                           mTextureSampleSamplerCreateResult             = VK_SUCCESS;
    VkResult                           mTextureSampleDescriptorSetLayoutCreateResult = VK_SUCCESS;
    VkResult                           mTextureSamplePipelineLayoutCreateResult      = VK_SUCCESS;
    VkResult                           mTextureSampleDescriptorPoolCreateResult      = VK_SUCCESS;
    VkResult                           mTextureSampleDescriptorSetAllocateResult     = VK_SUCCESS;
    bool                               mNullTextureSampleSampler                     = false;
    bool                               mNullTextureSampleDescriptorSetLayout         = false;
    bool                               mNullTextureSamplePipelineLayout              = false;
    bool                               mNullTextureSampleDescriptorPool              = false;
    bool                               mNullTextureSampleDescriptorSet               = false;
    std::size_t                        mCreateTextureSampleSamplerCalls              = 0;
    std::size_t                        mDestroyTextureSampleSamplerCalls             = 0;
    std::size_t                        mCreateTextureSampleDescriptorSetLayoutCalls  = 0;
    std::size_t                        mDestroyTextureSampleDescriptorSetLayoutCalls = 0;
    std::size_t                        mCreateTextureSamplePipelineLayoutCalls       = 0;
    std::size_t                        mDestroyTextureSamplePipelineLayoutCalls      = 0;
    std::size_t                        mCreateTextureSampleDescriptorPoolCalls       = 0;
    std::size_t                        mDestroyTextureSampleDescriptorPoolCalls      = 0;
    std::size_t                        mAllocateTextureSampleDescriptorSetCalls      = 0;
    std::size_t                        mUpdateTextureSampleDescriptorSetCalls        = 0;
    VkSamplerCreateInfo                mTextureSampleSamplerCreateInfo{};
    VkDescriptorSetLayoutBinding       mTextureSampleLayoutBinding{};
    VkDescriptorSetLayoutCreateInfo    mTextureSampleDescriptorSetLayoutCreateInfo{};
    VkPipelineLayoutCreateInfo         mTextureSamplePipelineLayoutCreateInfo{};
    VkDescriptorSetLayout              mTextureSamplePipelineSetLayout = VK_NULL_HANDLE;
    VkDescriptorPoolSize               mTextureSampleDescriptorPoolSize{};
    VkDescriptorPoolCreateInfo         mTextureSampleDescriptorPoolCreateInfo{};
    VkDescriptorSetAllocateInfo        mTextureSampleDescriptorSetAllocateInfo{};
    VkDescriptorSetLayout              mTextureSampleAllocatedSetLayout = VK_NULL_HANDLE;
    VkWriteDescriptorSet               mTextureSampleDescriptorWrite{};
    VkDescriptorImageInfo              mTextureSampleDescriptorImageInfo{};
    std::vector<VkSampler>             mDestroyedTextureSampleSamplers;
    ...
    VkMemoryRequirements             mTextureUploadSourceMemoryRequirements{};
    ...
    bool                             mNullTextureUploadSourceBuffer    = false;
    bool                             mNullTextureUploadSourceMemory    = false;
    bool                             mNullTextureUploadSourceMapping   = false;
    ...
    VkBufferCreateInfo               mTextureUploadSourceBufferCreateInfo{};
    ...
    VkMemoryAllocateInfo             mTextureUploadSourceMemoryAllocateInfo{};
    ...
    VkBuffer                         mTextureUploadSourceBoundBuffer = VK_NULL_HANDLE;
    VkDeviceMemory                   mTextureUploadSourceBoundMemory = VK_NULL_HANDLE;
    VkDeviceSize                     mTextureUploadSourceBindOffset  = std::numeric_limits<VkDeviceSize>::max();
    ...
    VulkanInstanceGeneration*                           mTextureUploadSourceTeardownReentryOwner  = nullptr;
    bool                                                mTextureUploadSourceTeardownReentryActive = false;
    std::vector<TextureUploadSourceTeardownObservation> mTextureUploadSourceTeardownObservations;
    VulkanInstanceGeneration*                                      mTextureUploadTransferOperationReentryOwner   = nullptr;
    bool                                                           mTextureUploadTransferOperationReentryInvoked = false;
    std::optional<VulkanTextureUploadTransferParentOperationError> mTextureUploadTransferOperationReentryError;
    VulkanInstanceGeneration*                                      mTextureUploadTransferTeardownOwner  = nullptr;
    bool                                                           mTextureUploadTransferTeardownActive = false;
    std::vector<TextureUploadTransferTeardownObservation>          mTextureUploadTransferTeardownObservations;
    VulkanInstanceGeneration*                                      mTextureUploadDestinationTeardownOwner  = nullptr;
    bool                                                           mTextureUploadDestinationTeardownActive = false;
    std::vector<TextureUploadDestinationTeardownObservation>       mTextureUploadDestinationTeardownObservations;
    ...
    FakeState()
    {
        ...
        mTextureFormatProperties.optimalTilingFeatures = VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT | VK_FORMAT_FEATURE_TRANSFER_SRC_BIT |
        ...
        mTextureImageFormatProperties.maxExtent             = { 4096, 4096, 1 };
        mTextureImageFormatProperties.maxMipLevels          = 12;
        mTextureImageFormatProperties.maxArrayLayers        = 1;
        mTextureImageFormatProperties.sampleCounts          = VK_SAMPLE_COUNT_1_BIT;
        mTextureImageFormatProperties.maxResourceSize       = 64 * 1024 * 1024;
        ...
        mTextureUploadSourceMemoryRequirements.size           = 512;
        mTextureUploadSourceMemoryRequirements.alignment      = 16;
        mTextureUploadSourceMemoryRequirements.memoryTypeBits = 1;
        ...
        mTextureImageMemoryRequirements.size                = 4096;
        mTextureImageMemoryRequirements.alignment           = 256;
        mTextureImageMemoryRequirements.memoryTypeBits      = 3;
    }
};

FakeState* gFakeState = nullptr;

void attemptTextureUploadSourceTeardownReentry(FakeState& state) noexcept;
                                              ...
                                              const VkAllocationCallbacks* allocation_callbacks) noexcept
{
    ...
    if (gFakeState->mSurfaceDestroyOwner)
    {
        ...
        gFakeState->mObservedTextureUploadDestinationAtSurfaceDestroy =
            gFakeState->mSurfaceDestroyOwner->hasTextureUploadDestinationGeneration();
        gFakeState->mObservedTextureUploadSourceAtSurfaceDestroy = gFakeState->mSurfaceDestroyOwner->hasTextureUploadSourceGeneration();
        gFakeState->mObservedTextureUploadSampleBindingAtSurfaceDestroy =
            gFakeState->mSurfaceDestroyOwner->hasTextureUploadSampleBindingGeneration();
    }
...
VKAPI_ATTR void VKAPI_CALL fakeDestroyDevice(VkDevice device, const VkAllocationCallbacks*) noexcept
{
    ...
    if (gFakeState->mDeviceDestroyOwner)
    {
        ...
        gFakeState->mObservedTextureUploadDestinationAtDeviceDestroy =
            gFakeState->mDeviceDestroyOwner->hasTextureUploadDestinationGeneration();
        gFakeState->mObservedTextureUploadSourceAtDeviceDestroy = gFakeState->mDeviceDestroyOwner->hasTextureUploadSourceGeneration();
        gFakeState->mObservedTextureUploadSampleBindingAtDeviceDestroy =
            gFakeState->mDeviceDestroyOwner->hasTextureUploadSampleBindingGeneration();
                                                                 ...
                                                                 VkFormatProperties* properties) noexcept
{
    ...
    const bool swapchain_format = !gFakeState->mSwapchainFormats.empty() && format == gFakeState->mSwapchainFormats.front().format;
    const bool texture_format   = format == VK_FORMAT_R8G8B8A8_UNORM;
    if (!swapchain_format && !texture_format)
    {
        return;
    }
    ...
    *properties = {};
    if (swapchain_format)
    {
        ...
        properties->linearTilingFeatures |= gFakeState->mSwapchainFormatProperties.linearTilingFeatures;
        properties->optimalTilingFeatures |= gFakeState->mSwapchainFormatProperties.optimalTilingFeatures;
        properties->bufferFeatures |= gFakeState->mSwapchainFormatProperties.bufferFeatures;
    }
    if (texture_format)
    {
        ++gFakeState->mTextureFormatPropertiesCalls;
        gFakeState->mTextureFormatPropertiesDevice = physical_device;
        gFakeState->mTextureFormatPropertiesFormat = format;
        properties->linearTilingFeatures |= gFakeState->mTextureFormatProperties.linearTilingFeatures;
        properties->optimalTilingFeatures |= gFakeState->mTextureFormatProperties.optimalTilingFeatures;
        properties->bufferFeatures |= gFakeState->mTextureFormatProperties.bufferFeatures;
    }
}
                                                                          ...
                                                                          VkImageFormatProperties* properties) noexcept
{
    ...
    ++gFakeState->mTextureImageFormatPropertiesCalls;
    gFakeState->mTextureImageFormatPropertiesDevice = physical_device;
    gFakeState->mTextureImageFormat                 = format;
    gFakeState->mTextureImageType                   = image_type;
    gFakeState->mTextureImageTiling                 = tiling;
    gFakeState->mTextureImageUsage                  = usage;
    gFakeState->mTextureImageFlags                  = flags;
    if (gFakeState->mTextureImageFormatResult == VK_SUCCESS)
    {
        *properties = gFakeState->mTextureImageFormatProperties;
    }
    return gFakeState->mTextureImageFormatResult;
}
                                               ...
                                               VkImage*                     image) noexcept
{
    ...
    ++gFakeState->mCreateTextureImageCalls;
    gFakeState->mTextureImageCreateInfo = *create_info;
    gFakeState->mCreateTextureImageAllocationCallbacks.push_back(allocation_callbacks);
    if (gFakeState->mTextureImageCreateResult == VK_SUCCESS)
    {
        *image = gFakeState->mNullTextureImage ? VK_NULL_HANDLE : gFakeState->mTextureUploadDestinationImage;
        if (*image != VK_NULL_HANDLE)
        {
            gFakeState->mCreatedTextureImages.push_back(*image);
            gFakeState->mEvents.push_back(Event::CreateTextureImage);
        }
    }
    return gFakeState->mTextureImageCreateResult;
}
...
{
    if (!gFakeState || device != gFakeState->mDevice ||
        std::find(gFakeState->mCreatedTextureImages.begin(), gFakeState->mCreatedTextureImages.end(), image) ==
            gFakeState->mCreatedTextureImages.end())
    {
        return;
    }
    ++gFakeState->mDestroyTextureImageCalls;
    attemptTextureUploadDestinationTeardownReentry(*gFakeState);
    gFakeState->mDestroyedTextureImages.push_back(image);
    gFakeState->mDestroyTextureImageAllocationCallbacks.push_back(allocation_callbacks);
    gFakeState->mEvents.push_back(Event::DestroyTextureImage);
}
                                                   ...
                                                   VkImageView*                 image_view) noexcept
{
    ...
    const bool texture_image = std::find(gFakeState->mCreatedTextureImages.begin(), gFakeState->mCreatedTextureImages.end(),
                                         create_info->image) != gFakeState->mCreatedTextureImages.end();
    if (texture_image)
    {
        ++gFakeState->mCreateTextureImageViewCalls;
        gFakeState->mTextureImageViewCreateInfo = *create_info;
        gFakeState->mCreateTextureImageViewAllocationCallbacks.push_back(allocation_callbacks);
        if (gFakeState->mTextureImageViewCreateResult == VK_SUCCESS)
        {
            *image_view = gFakeState->mNullTextureImageView ? VK_NULL_HANDLE : gFakeState->mTextureUploadDestinationView;
            if (*image_view != VK_NULL_HANDLE)
            {
                gFakeState->mCreatedTextureImageViews.push_back(*image_view);
                gFakeState->mEvents.push_back(Event::CreateTextureImageView);
            }
        }
        return gFakeState->mTextureImageViewCreateResult;
    }
                                                ...
                                                const VkAllocationCallbacks* allocation_callbacks) noexcept
{
    ...
    if (std::find(gFakeState->mCreatedTextureImageViews.begin(), gFakeState->mCreatedTextureImageViews.end(), image_view) !=
        gFakeState->mCreatedTextureImageViews.end())
    {
        ++gFakeState->mDestroyTextureImageViewCalls;
        attemptTextureUploadDestinationTeardownReentry(*gFakeState);
        gFakeState->mDestroyedTextureImageViews.push_back(image_view);
        gFakeState->mDestroyTextureImageViewAllocationCallbacks.push_back(allocation_callbacks);
        gFakeState->mEvents.push_back(Event::DestroyTextureImageView);
        return;
    }
...
{
    if (!gFakeState || device != gFakeState->mDevice || !create_info || !sampler ||
        create_info->sType != VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO)
    {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ++gFakeState->mCreateTextureSampleSamplerCalls;
    gFakeState->mTextureSampleSamplerCreateInfo = *create_info;
    gFakeState->mEvents.push_back(Event::CreateTextureSampleSampler);
    if (gFakeState->mTextureSampleSamplerCreateResult == VK_SUCCESS)
    {
        *sampler = gFakeState->mNullTextureSampleSampler ? VK_NULL_HANDLE : gFakeState->mTextureSampleSampler;
    }
    return gFakeState->mTextureSampleSamplerCreateResult;
}

VKAPI_ATTR void VKAPI_CALL fakeDestroySampler(VkDevice, VkSampler sampler, const VkAllocationCallbacks*) noexcept
{
    if (!gFakeState)
    {
        return;
    }
    ++gFakeState->mDestroyTextureSampleSamplerCalls;
    gFakeState->mDestroyedTextureSampleSamplers.push_back(sampler);
    gFakeState->mEvents.push_back(Event::DestroyTextureSampleSampler);
    attemptTextureUploadSampleBindingTeardownReentry(*gFakeState);
}
...
{
    if (!gFakeState || device != gFakeState->mDevice || !create_info || !descriptor_set_layout ||
        create_info->sType != VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO || create_info->bindingCount != 1 ||
        !create_info->pBindings)
    {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ++gFakeState->mCreateTextureSampleDescriptorSetLayoutCalls;
    gFakeState->mTextureSampleDescriptorSetLayoutCreateInfo = *create_info;
    gFakeState->mTextureSampleLayoutBinding                 = create_info->pBindings[0];
    gFakeState->mEvents.push_back(Event::CreateTextureSampleDescriptorSetLayout);
    if (gFakeState->mTextureSampleDescriptorSetLayoutCreateResult == VK_SUCCESS)
    {
        *descriptor_set_layout =
            gFakeState->mNullTextureSampleDescriptorSetLayout ? VK_NULL_HANDLE : gFakeState->mTextureSampleDescriptorSetLayout;
    }
    return gFakeState->mTextureSampleDescriptorSetLayoutCreateResult;
}

VKAPI_ATTR void VKAPI_CALL fakeDestroyDescriptorSetLayout(VkDevice,
                                                          ...
                                                          const VkAllocationCallbacks*) noexcept
{
    ...
    ++gFakeState->mDestroyTextureSampleDescriptorSetLayoutCalls;
    gFakeState->mDestroyedTextureSampleDescriptorSetLayouts.push_back(descriptor_set_layout);
    gFakeState->mEvents.push_back(Event::DestroyTextureSampleDescriptorSetLayout);
    attemptTextureUploadSampleBindingTeardownReentry(*gFakeState);
}
...
{
    if (!gFakeState || device != gFakeState->mDevice || !create_info || !descriptor_pool ||
        create_info->sType != VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO || create_info->poolSizeCount != 1 || !create_info->pPoolSizes)
    {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ++gFakeState->mCreateTextureSampleDescriptorPoolCalls;
    gFakeState->mTextureSampleDescriptorPoolCreateInfo = *create_info;
    gFakeState->mTextureSampleDescriptorPoolSize       = create_info->pPoolSizes[0];
    gFakeState->mEvents.push_back(Event::CreateTextureSampleDescriptorPool);
    if (gFakeState->mTextureSampleDescriptorPoolCreateResult == VK_SUCCESS)
    {
        *descriptor_pool = gFakeState->mNullTextureSampleDescriptorPool ? VK_NULL_HANDLE : gFakeState->mTextureSampleDescriptorPool;
    }
    return gFakeState->mTextureSampleDescriptorPoolCreateResult;
}

VKAPI_ATTR void VKAPI_CALL fakeDestroyDescriptorPool(VkDevice, VkDescriptorPool descriptor_pool, const VkAllocationCallbacks*) noexcept
{
    if (!gFakeState)
    {
        return;
    }
    ++gFakeState->mDestroyTextureSampleDescriptorPoolCalls;
    gFakeState->mDestroyedTextureSampleDescriptorPools.push_back(descriptor_pool);
    gFakeState->mEvents.push_back(Event::DestroyTextureSampleDescriptorPool);
    attemptTextureUploadSampleBindingTeardownReentry(*gFakeState);
}
...
{
    if (!gFakeState || device != gFakeState->mDevice || !allocate_info || !descriptor_sets ||
        allocate_info->sType != VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO || allocate_info->descriptorSetCount != 1 ||
        !allocate_info->pSetLayouts)
    {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ++gFakeState->mAllocateTextureSampleDescriptorSetCalls;
    gFakeState->mTextureSampleDescriptorSetAllocateInfo = *allocate_info;
    gFakeState->mTextureSampleAllocatedSetLayout        = allocate_info->pSetLayouts[0];
    gFakeState->mEvents.push_back(Event::AllocateTextureSampleDescriptorSet);
    if (gFakeState->mTextureSampleDescriptorSetAllocateResult == VK_SUCCESS)
    {
        *descriptor_sets = gFakeState->mNullTextureSampleDescriptorSet ? VK_NULL_HANDLE : gFakeState->mTextureSampleDescriptorSet;
    }
    return gFakeState->mTextureSampleDescriptorSetAllocateResult;
}
...
{
    if (!gFakeState || device != gFakeState->mDevice || descriptor_write_count != 1 || !descriptor_writes || descriptor_copy_count != 0 ||
        descriptor_copies)
    {
        return;
    }
    ++gFakeState->mUpdateTextureSampleDescriptorSetCalls;
    gFakeState->mTextureSampleDescriptorWrite = descriptor_writes[0];
    gFakeState->mTextureSampleDescriptorImageInfo =
        descriptor_writes[0].pImageInfo ? descriptor_writes[0].pImageInfo[0] : VkDescriptorImageInfo{};
    gFakeState->mEvents.push_back(Event::UpdateTextureSampleDescriptorSet);
}
                                                        ...
                                                        VkPipelineLayout* pipeline_layout) noexcept
{
    ...
    {
        ++gFakeState->mCreateTextureSamplePipelineLayoutCalls;
        gFakeState->mTextureSamplePipelineLayoutCreateInfo = *create_info;
        gFakeState->mTextureSamplePipelineSetLayout        = create_info->pSetLayouts[0];
        gFakeState->mEvents.push_back(Event::CreateTextureSamplePipelineLayout);
        if (gFakeState->mTextureSamplePipelineLayoutCreateResult == VK_SUCCESS)
        {
            *pipeline_layout = gFakeState->mNullTextureSamplePipelineLayout ? VK_NULL_HANDLE : gFakeState->mTextureSamplePipelineLayout;
        }
        return gFakeState->mTextureSamplePipelineLayoutCreateResult;
    }
                                                     ...
                                                     const VkAllocationCallbacks*) noexcept
{
    ...
    if (pipeline_layout == gFakeState->mTextureSamplePipelineLayout)
    {
        ++gFakeState->mDestroyTextureSamplePipelineLayoutCalls;
        gFakeState->mDestroyedTextureSamplePipelineLayouts.push_back(pipeline_layout);
        gFakeState->mEvents.push_back(Event::DestroyTextureSamplePipelineLayout);
        attemptTextureUploadSampleBindingTeardownReentry(*gFakeState);
    }
    else
    {
                                                  ...
                                                  const VkAllocationCallbacks* allocation_callbacks) noexcept
{
    ...
    {
        if (gFakeState->mTextureUploadTransferTeardownOwner)
        {
            attemptTextureUploadTransferTeardownReentry(*gFakeState);
        }
                                               ...
                                               VkFence             fence) noexcept
{
    ...
    attemptTextureUploadTransferOperationReentry(*gFakeState);
...
{
    if (!gFakeState || command_buffer != gFakeState->mUploadTransferCommandBuffer ||
        source_buffer != gFakeState->mTextureUploadSourceBuffer || destination_image != gFakeState->mTextureUploadDestinationImage ||
        destination_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL || region_count != 1 || !regions)
    {
        return;
    }
    ++gFakeState->mCopyBufferCalls;
...
{
    if (!gFakeState || command_buffer != gFakeState->mUploadTransferCommandBuffer ||
        source_image != gFakeState->mTextureUploadDestinationImage || destination_image != source_image ||
        source_layout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL || destination_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL ||
        region_count != 1 || !regions || filter != VK_FILTER_LINEAR)
    {
        return;
    }
}
...
{
    if (!gFakeState || device != gFakeState->mDevice || !requirements_info || !requirements ||
        requirements_info->sType != VK_STRUCTURE_TYPE_IMAGE_MEMORY_REQUIREMENTS_INFO_2 ||
        std::find(gFakeState->mCreatedTextureImages.begin(), gFakeState->mCreatedTextureImages.end(), requirements_info->image) ==
            gFakeState->mCreatedTextureImages.end())
    {
        return;
    }
    ++gFakeState->mGetTextureImageMemoryRequirementsCalls;
    gFakeState->mTextureMemoryRequirementsImage = requirements_info->image;
    requirements->memoryRequirements            = gFakeState->mTextureImageMemoryRequirements;
    if (auto* dedicated = static_cast<VkMemoryDedicatedRequirements*>(requirements->pNext);
        dedicated && dedicated->sType == VK_STRUCTURE_TYPE_MEMORY_DEDICATED_REQUIREMENTS)
    {
        dedicated->prefersDedicatedAllocation  = gFakeState->mTexturePrefersDedicatedAllocation;
        dedicated->requiresDedicatedAllocation = gFakeState->mTextureRequiresDedicatedAllocation;
    }
    gFakeState->mNextBufferMemoryKind = BufferMemoryKind::TextureUploadDestination;
}
                                                ...
                                                VkBuffer* buffer) noexcept
{
    ...
    const bool texture_upload_source =
        create_info->size == VULKAN_TEXTURE_UPLOAD_SOURCE_BYTE_COUNT && create_info->usage == VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
    ...
    if (texture_upload_source)
    {
        gFakeState->mTextureUploadSourceBufferCreateInfo = *create_info;
        *buffer = gFakeState->mNullTextureUploadSourceBuffer ? VK_NULL_HANDLE : gFakeState->mTextureUploadSourceBuffer;
    }
...
VKAPI_ATTR void VKAPI_CALL fakeDestroyBuffer(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks*) noexcept
{
        ...
        std::find(gFakeState->mCreatedBuffers.begin(), gFakeState->mCreatedBuffers.end(), buffer) != gFakeState->mCreatedBuffers.end())
    {
        ...
        else if (buffer == gFakeState->mTextureUploadSourceBuffer)
        {
            gFakeState->mEvents.push_back(Event::DestroyTextureUploadSourceBuffer);
            attemptTextureUploadSourceTeardownReentry(*gFakeState);
        }
...
VKAPI_ATTR void VKAPI_CALL fakeGetBufferMemoryRequirements(VkDevice device, VkBuffer buffer, VkMemoryRequirements* requirements) noexcept
{
        ...
        std::find(gFakeState->mCreatedBuffers.begin(), gFakeState->mCreatedBuffers.end(), buffer) != gFakeState->mCreatedBuffers.end())
    {
        ...
        gFakeState->mNextBufferMemoryKind =
            buffer == gFakeState->mTextureUploadSourceBuffer
                ? BufferMemoryKind::TextureUploadSource
                : (buffer == gFakeState->mUploadSourceBuffer
                       ? BufferMemoryKind::UploadSource
                       : (buffer == gFakeState->mUploadDestinationBuffer ? BufferMemoryKind::UploadDestination
                                                                         : BufferMemoryKind::Readback));
        if (gFakeState->mNextBufferMemoryKind == BufferMemoryKind::TextureUploadSource)
        {
            *requirements      = gFakeState->mTextureUploadSourceMemoryRequirements;
            requirements->size = std::max(requirements->size, gFakeState->mTextureUploadSourceBufferCreateInfo.size);
        }
        else if (gFakeState->mNextBufferMemoryKind == BufferMemoryKind::UploadSource)
        {
                                                  ...
                                                  VkDeviceMemory*              memory) noexcept
{
    ...
    VkResult result                    = gFakeState->mMemoryAllocateResult;
    if (memory_kind == BufferMemoryKind::TextureUploadDestination)
    {
        ++gFakeState->mAllocateTextureMemoryCalls;
        gFakeState->mTextureMemoryAllocateInfo       = *allocate_info;
        gFakeState->mTextureMemoryAllocateInfo.pNext = nullptr;
        gFakeState->mAllocateTextureMemoryAllocationCallbacks.push_back(allocation_callbacks);
        if (const auto* dedicated = static_cast<const VkMemoryDedicatedAllocateInfo*>(allocate_info->pNext);
            dedicated && dedicated->sType == VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO)
        {
            gFakeState->mTextureDedicatedAllocateInfo = *dedicated;
        }
        result = gFakeState->mTextureMemoryAllocateResult;
        if (result == VK_SUCCESS)
        {
            *memory = gFakeState->mNullTextureMemory ? VK_NULL_HANDLE : gFakeState->mTextureUploadDestinationMemory;
        }
    }
    else if (memory_kind == BufferMemoryKind::UploadSource)
    {
    ...
    else if (memory_kind == BufferMemoryKind::TextureUploadSource)
    {
        gFakeState->mTextureUploadSourceMemoryAllocateInfo = *allocate_info;
        *memory = gFakeState->mNullTextureUploadSourceMemory ? VK_NULL_HANDLE : gFakeState->mTextureUploadSourceMemory;
    }
    ...
    if (result == VK_SUCCESS && *memory != VK_NULL_HANDLE)
    {
        ...
        if (memory_kind == BufferMemoryKind::TextureUploadDestination)
        {
            gFakeState->mAllocatedTextureMemories.push_back(*memory);
            gFakeState->mEvents.push_back(Event::AllocateTextureMemory);
        }
    }
    return result;
}
                                          ...
                                          const VkAllocationCallbacks* allocation_callbacks) noexcept
{
            ...
            gFakeState->mAllocatedMemories.end())
    {
        ...
        if (std::find(gFakeState->mAllocatedTextureMemories.begin(), gFakeState->mAllocatedTextureMemories.end(), memory) !=
            gFakeState->mAllocatedTextureMemories.end())
        {
            ++gFakeState->mFreeTextureMemoryCalls;
            attemptTextureUploadDestinationTeardownReentry(*gFakeState);
            gFakeState->mFreedTextureMemories.push_back(memory);
            gFakeState->mFreeTextureMemoryAllocationCallbacks.push_back(allocation_callbacks);
            gFakeState->mEvents.push_back(Event::FreeTextureMemory);
        }
        else if (memory == gFakeState->mUploadSourceMemory)
        {
            gFakeState->mEvents.push_back(Event::FreeUploadSourceMemory);
        }
        else if (memory == gFakeState->mTextureUploadSourceMemory)
        {
            gFakeState->mEvents.push_back(Event::FreeTextureUploadSourceMemory);
            attemptTextureUploadSourceTeardownReentry(*gFakeState);
        }
...
VKAPI_ATTR VkResult VKAPI_CALL fakeBindImageMemory(VkDevice       device,
                                                   VkImage        image,
                                                   VkDeviceMemory memory,
                                                   VkDeviceSize   memory_offset) noexcept
{
    if (!gFakeState || device != gFakeState->mDevice ||
        std::find(gFakeState->mCreatedTextureImages.begin(), gFakeState->mCreatedTextureImages.end(), image) ==
            gFakeState->mCreatedTextureImages.end() ||
        std::find(gFakeState->mAllocatedTextureMemories.begin(), gFakeState->mAllocatedTextureMemories.end(), memory) ==
            gFakeState->mAllocatedTextureMemories.end())
    {
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    ++gFakeState->mBindTextureImageMemoryCalls;
    gFakeState->mTextureBoundImage  = image;
    gFakeState->mTextureBoundMemory = memory;
    gFakeState->mTextureBindOffset  = memory_offset;
    return gFakeState->mTextureImageBindResult;
}
                                                    ...
                                                    VkDeviceSize   memory_offset) noexcept
{
    if (!gFakeState || device != gFakeState->mDevice ||
        !((buffer == gFakeState->mReadbackBuffer && memory == gFakeState->mReadbackMemory) ||
          (buffer == gFakeState->mTextureUploadSourceBuffer && memory == gFakeState->mTextureUploadSourceMemory) ||
    ...
    if (buffer == gFakeState->mTextureUploadSourceBuffer)
    {
        gFakeState->mTextureUploadSourceBoundBuffer = buffer;
        gFakeState->mTextureUploadSourceBoundMemory = memory;
        gFakeState->mTextureUploadSourceBindOffset  = memory_offset;
    }
    else if (buffer == gFakeState->mUploadSourceBuffer)
    {
    ...
    fakeMapMemory(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags, void** data) noexcept
{
    ...
    {
        *data = gFakeState->mNullTextureUploadSourceMapping ? nullptr : gFakeState->mTextureUploadSourceMappedStorage.data();
    }
    else
    {
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL fakeGetDeviceProcAddr(VkDevice device, const char* name) noexcept
{
    ...
    {
        return gFakeState->mMissing == MissingCommand::CreateImage ? nullptr : eraseFunctionType(fakeCreateImage);
    }
    if (std::strcmp(name, "vkDestroyImage") == 0)
    {
        return gFakeState->mMissing == MissingCommand::DestroyImage ? nullptr : eraseFunctionType(fakeDestroyImage);
    }
    if (std::strcmp(name, "vkGetImageMemoryRequirements2") == 0)
    {
        return gFakeState->mMissing == MissingCommand::GetImageMemoryRequirements2 ? nullptr
                                                                                   : eraseFunctionType(fakeGetImageMemoryRequirements2);
    }
    if (std::strcmp(name, "vkBindImageMemory") == 0)
    {
        return gFakeState->mMissing == MissingCommand::BindImageMemory ? nullptr : eraseFunctionType(fakeBindImageMemory);
    }
    ...
    {
        return gFakeState->mMissing == MissingCommand::CreateSampler ? nullptr : eraseFunctionType(fakeCreateSampler);
    }
    if (std::strcmp(name, "vkDestroySampler") == 0)
    {
        return gFakeState->mMissing == MissingCommand::DestroySampler ? nullptr : eraseFunctionType(fakeDestroySampler);
    }
    if (std::strcmp(name, "vkCreateDescriptorSetLayout") == 0)
    {
        return gFakeState->mMissing == MissingCommand::CreateDescriptorSetLayout ? nullptr
                                                                                 : eraseFunctionType(fakeCreateDescriptorSetLayout);
    }
    if (std::strcmp(name, "vkDestroyDescriptorSetLayout") == 0)
    {
        return gFakeState->mMissing == MissingCommand::DestroyDescriptorSetLayout ? nullptr
                                                                                  : eraseFunctionType(fakeDestroyDescriptorSetLayout);
    }
    if (std::strcmp(name, "vkCreateDescriptorPool") == 0)
    {
        return gFakeState->mMissing == MissingCommand::CreateDescriptorPool ? nullptr : eraseFunctionType(fakeCreateDescriptorPool);
    }
    if (std::strcmp(name, "vkDestroyDescriptorPool") == 0)
    {
        return gFakeState->mMissing == MissingCommand::DestroyDescriptorPool ? nullptr : eraseFunctionType(fakeDestroyDescriptorPool);
    }
    if (std::strcmp(name, "vkAllocateDescriptorSets") == 0)
    {
        return gFakeState->mMissing == MissingCommand::AllocateDescriptorSets ? nullptr : eraseFunctionType(fakeAllocateDescriptorSets);
    }
    if (std::strcmp(name, "vkUpdateDescriptorSets") == 0)
    {
        return gFakeState->mMissing == MissingCommand::UpdateDescriptorSets ? nullptr : eraseFunctionType(fakeUpdateDescriptorSets);
    }
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL fakeGetInstanceProcAddr(VkInstance instance, const char* name) noexcept
{
    ...
    {
        return gFakeState->mMissing == MissingCommand::GetPhysicalDeviceImageFormatProperties
                   ? nullptr
...
constexpr VulkanTextureUploadSourceBytes makeTextureUploadSourceBytes() noexcept
{
    VulkanTextureUploadSourceBytes bytes{};
    for (std::size_t top_row = 0; top_row < LLRenderContract::TEXTURE_UPLOAD_RESIDENT_HEIGHT; ++top_row)
    {
        const std::size_t row_start = top_row * LLRenderContract::TEXTURE_UPLOAD_ROW_PITCH;
        for (std::size_t x = 0; x < LLRenderContract::TEXTURE_UPLOAD_RESIDENT_WIDTH; ++x)
        {
            for (std::size_t channel = 0; channel < LLRenderContract::TEXTURE_UPLOAD_CHANNELS; ++channel)
            {
                bytes[row_start + x * LLRenderContract::TEXTURE_UPLOAD_CHANNELS + channel] =
                    static_cast<std::uint8_t>(5 + channel * 64 + top_row * 8 + x * 2);
            }
        }
        for (std::size_t padding = LLRenderContract::TEXTURE_UPLOAD_RESIDENT_WIDTH * LLRenderContract::TEXTURE_UPLOAD_CHANNELS;
             padding < LLRenderContract::TEXTURE_UPLOAD_ROW_PITCH;
             ++padding)
        {
            bytes[row_start + padding] =
                static_cast<std::uint8_t>(0xf0U + top_row * 4 + padding -
                                          LLRenderContract::TEXTURE_UPLOAD_RESIDENT_WIDTH * LLRenderContract::TEXTURE_UPLOAD_CHANNELS);
        }
    }
    return bytes;
}

constexpr VulkanTextureUploadSourceDescription makeTextureUploadSourceDescription() noexcept
{
    return vulkanTextureUploadSourceDescription(makeTextureUploadSourceBytes());
}
...
void prepareTextureUploadDestinationState(FakeState& state) noexcept
{
    state.mMemoryProperties.memoryTypeCount              = 2;
    state.mMemoryProperties.memoryTypes[1].propertyFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
    state.mMemoryProperties.memoryTypes[1].heapIndex     = 0;
}

VulkanTextureUploadDestinationRequest makeTextureUploadDestinationRequest(
    FakeState&                                state,
    VulkanInstanceGeneration&                 owner,
    VulkanTextureUploadDestinationDescription description = vulkanTextureUploadDestinationDescription()) noexcept
{
    state.mExpectedInstanceOwner = &owner;
    return { 42, description, { &state, instanceOwnerIsCurrent }, { &state, surfaceWindowIsCurrent } };
}

VulkanTextureUploadSourceRequest makeTextureUploadSourceRequest(
    FakeState&                           state,
    VulkanInstanceGeneration&            owner,
    VulkanTextureUploadSourceDescription description = makeTextureUploadSourceDescription()) noexcept
{
    state.mExpectedInstanceOwner = &owner;
    return { 42, description, { &state, instanceOwnerIsCurrent }, { &state, surfaceWindowIsCurrent } };
}

VulkanTextureUploadTransferRequest makeTextureUploadTransferRequest(FakeState& state, VulkanInstanceGeneration& owner) noexcept
{
    state.mExpectedInstanceOwner = &owner;
    return { 42,
...
VulkanTextureUploadTransferOperationRequest makeTextureUploadTransferOperationRequest(FakeState& state, VulkanInstanceGeneration& owner,
                                                                                      std::uint64_t timeout_ns = 1'000'000) noexcept
{
    state.mExpectedInstanceOwner = &owner;
    return { 42,         makeTextureUploadSourceDescription(), vulkanTextureUploadDestinationDescription(),
             timeout_ns, { &state, instanceOwnerIsCurrent },   { &state, surfaceWindowIsCurrent } };
}

VulkanTextureUploadSampleBindingRequest makeTextureUploadSampleBindingRequest(
    FakeState&                                  state,
    VulkanInstanceGeneration&                   owner,
    VulkanTextureUploadSampleBindingDescription description = vulkanTextureUploadSampleBindingDescription()) noexcept
{
    state.mExpectedInstanceOwner = &owner;
    return { 42,
             vulkanTextureUploadDestinationDescription(),
...
void attemptTextureUploadTransferOperationReentry(FakeState& state) noexcept
{
    ...
    state.mTextureUploadTransferOperationReentryInvoked = true;
    const auto result                                   = state.mTextureUploadTransferOperationReentryOwner->executeTextureUploadTransfer(
        makeTextureUploadTransferOperationRequest(state, *state.mTextureUploadTransferOperationReentryOwner));
    if (const auto* error = std::get_if<VulkanTextureUploadTransferParentOperationError>(&result))
    {
        state.mTextureUploadTransferOperationReentryError = *error;
    }
}
...
void attemptTextureUploadTransferTeardownReentry(FakeState& state) noexcept
{
    auto* owner = state.mTextureUploadTransferTeardownOwner;
    if (!owner || state.mTextureUploadTransferTeardownActive)
    {
        return;
    }
    state.mTextureUploadTransferTeardownActive = true;
    TextureUploadTransferTeardownObservation observation;
    observation.mTransferDetached = !owner->hasTextureUploadTransferGeneration() && !owner->textureUploadTransferDisposition() &&
                                    owner->textureUploadTransferCommandPool() == VK_NULL_HANDLE &&
                                    owner->textureUploadTransferFence() == VK_NULL_HANDLE;
    const auto owner_checks   = state.mInstanceOwnerChecks;
    const auto window_checks  = state.mSurfaceWindowChecks;
    const auto pool_creates   = state.mCreateCommandPoolCalls;
    const auto submits        = state.mQueueSubmitCalls;
    const auto waits          = state.mWaitForFencesCalls;
    observation.mAcquireError = owner->acquireTextureUploadTransferGeneration(makeTextureUploadTransferRequest(state, *owner));
    ...
    const auto operation = owner->executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, *owner));
    if (const auto* error = std::get_if<VulkanTextureUploadTransferParentOperationError>(&operation))
    {
        observation.mOperationError = *error;
    }
    ...
    observation.mTransferResetSucceeded    = owner->resetTextureUploadTransferGeneration();
    observation.mSourceResetSucceeded      = owner->resetTextureUploadSourceGeneration();
    observation.mDestinationResetSucceeded = owner->resetTextureUploadDestinationGeneration();
    observation.mLogicalResetSucceeded     = owner->resetLogicalDeviceGeneration();
    observation.mFullResetSucceeded        = owner->reset();
    observation.mNoAuthOrNativeWork        = state.mInstanceOwnerChecks == owner_checks && state.mSurfaceWindowChecks == window_checks &&
                                      state.mCreateCommandPoolCalls == pool_creates && state.mQueueSubmitCalls == submits &&
                                      state.mWaitForFencesCalls == waits;
    state.mTextureUploadTransferTeardownObservations.push_back(std::move(observation));
    state.mTextureUploadTransferTeardownActive = false;
}

void attemptTextureUploadDestinationTeardownReentry(FakeState& state) noexcept
{
    auto* owner = state.mTextureUploadDestinationTeardownOwner;
    if (!owner || state.mTextureUploadDestinationTeardownActive)
    {
        return;
    }
    state.mTextureUploadDestinationTeardownActive = true;
    TextureUploadDestinationTeardownObservation observation;
    observation.mDestinationDetached = !owner->hasTextureUploadDestinationGeneration();
    ...
    VulkanInstanceGeneration moved(std::move(*owner));
    observation.mMoveProducedEmpty = moved.instance() == VK_NULL_HANDLE && owner->instance() != VK_NULL_HANDLE;
    const auto legacy_disposition  = owner->uploadTransferDisposition();
    const auto submit_calls        = state.mQueueSubmitCalls;
    const auto legacy_operation    = owner->executeUploadTransfer(makeUploadTransferOperationRequest(state, *owner));
    if (const auto* error = std::get_if<VulkanUploadTransferParentOperationError>(&legacy_operation))
    {
        observation.mLegacyOperationError = *error;
    }
    observation.mLegacyOperationUnchanged =
        owner->uploadTransferDisposition() == legacy_disposition && state.mQueueSubmitCalls == submit_calls;
    observation.mDestinationResetSucceeded = owner->resetTextureUploadDestinationGeneration();
    observation.mLogicalResetSucceeded     = owner->resetLogicalDeviceGeneration();
    observation.mFullResetSucceeded        = owner->reset();
    state.mTextureUploadDestinationTeardownObservations.push_back(std::move(observation));
    state.mTextureUploadDestinationTeardownActive = false;
}

void attemptTextureUploadSourceTeardownReentry(FakeState& state) noexcept
{
    VulkanInstanceGeneration* owner = state.mTextureUploadSourceTeardownReentryOwner;
    if (!owner || state.mTextureUploadSourceTeardownReentryActive)
    {
        return;
    }

    state.mTextureUploadSourceTeardownReentryActive = true;
    TextureUploadSourceTeardownObservation observation;
    observation.mSourceDetached                 = !owner->hasTextureUploadSourceGeneration();
    observation.mSwapchainConfigurationDetached = !owner->hasSwapchainConfigurationGeneration();
    observation.mDestinationLive                = owner->hasTextureUploadDestinationGeneration();
    observation.mLogicalDeviceLive              = owner->hasLogicalDeviceGeneration();
    observation.mSurfaceLive                    = owner->hasSurfaceGeneration();
    observation.mAcquireError = owner->acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, *owner));
    observation.mBindingAcquireError =
        owner->acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, *owner));

    const std::array configuration_native_calls = {
        state.mSwapchainCapabilitiesCalls,     state.mSwapchainFormatCountCalls,     state.mSwapchainFormatListCalls,
        state.mSwapchainPresentModeCountCalls, state.mSwapchainPresentModeListCalls, state.mSwapchainFormatPropertiesCalls,
    };
    observation.mConfigurationAcquireError =
        owner->acquireSwapchainConfigurationGeneration(makeSwapchainConfigurationRequest(state, *owner));
    observation.mConfigurationNativeWorkUnchanged =
        configuration_native_calls == std::array{ state.mSwapchainCapabilitiesCalls,    state.mSwapchainFormatCountCalls,
                                                  state.mSwapchainFormatListCalls,      state.mSwapchainPresentModeCountCalls,
                                                  state.mSwapchainPresentModeListCalls, state.mSwapchainFormatPropertiesCalls };

    const VulkanSwapchainChainRebuildResult rebuild_result =
    ...
    observation.mDestinationResetSucceeded = owner->resetTextureUploadDestinationGeneration();
    observation.mLogicalResetSucceeded     = owner->resetLogicalDeviceGeneration();
    observation.mSurfaceResetSucceeded     = owner->resetSurfaceGeneration();
    observation.mFullResetSucceeded        = owner->reset();
    state.mTextureUploadSourceTeardownObservations.push_back(std::move(observation));
    state.mTextureUploadSourceTeardownReentryActive = false;
}

void attemptTextureUploadSampleBindingTeardownReentry(FakeState& state) noexcept
{
    VulkanInstanceGeneration* owner = state.mTextureUploadSampleBindingTeardownOwner;
    if (!owner || state.mTextureUploadSampleBindingTeardownActive)
    {
        return;
    }

    state.mTextureUploadSampleBindingTeardownActive = true;
    TextureUploadSampleBindingTeardownObservation observation;
    observation.mBindingDetached = !owner->hasTextureUploadSampleBindingGeneration() &&
                                   owner->textureUploadSampleBindingSampler() == VK_NULL_HANDLE &&
                                   owner->textureUploadSampleBindingDescriptorSet() == VK_NULL_HANDLE;
    observation.mDestinationLive = owner->hasTextureUploadDestinationGeneration();
    observation.mSourceLive      = owner->hasTextureUploadSourceGeneration();
    observation.mTransferLive    = owner->hasTextureUploadTransferGeneration();
    observation.mBindingAcquireError =
        owner->acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, *owner));
    observation.mDestinationAcquireError =
        owner->acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, *owner));
    observation.mSourceAcquireError   = owner->acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, *owner));
    observation.mTransferAcquireError = owner->acquireTextureUploadTransferGeneration(makeTextureUploadTransferRequest(state, *owner));
    observation.mConfigurationAcquireError =
        owner->acquireSwapchainConfigurationGeneration(makeSwapchainConfigurationRequest(state, *owner));
    const auto rebuild = owner->rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, *owner, VkExtent2D{ 1440, 810 }));
    if (const auto* error = std::get_if<VulkanSwapchainChainRebuildError>(&rebuild))
...
const VulkanTextureUploadDestinationAcquireError& requireTextureUploadDestinationError(
    const VulkanTextureUploadDestinationAcquireResult& result)
{
    tut::ensure("texture-upload-destination acquisition returns an error", result.has_value());
    return *result;
}

void ensureTextureUploadDestinationCode(const VulkanTextureUploadDestinationAcquireResult& result,
                                        VulkanTextureUploadDestinationAcquireCode          code)
{
    tut::ensure("the exact texture-upload-destination error is reported", requireTextureUploadDestinationError(result).mCode == code);
}

const VulkanTextureUploadSourceAcquireError& requireTextureUploadSourceError(const VulkanTextureUploadSourceAcquireResult& result)
{
    tut::ensure("texture-upload-source acquisition returns an error", result.has_value());
    return *result;
}

void ensureTextureUploadSourceCode(const VulkanTextureUploadSourceAcquireResult& result, VulkanTextureUploadSourceAcquireCode code)
{
    tut::ensure("the exact texture-upload-source error is reported", requireTextureUploadSourceError(result).mCode == code);
}

const VulkanTextureUploadSampleBindingAcquireError& requireTextureUploadSampleBindingError(
    const VulkanTextureUploadSampleBindingAcquireResult& result)
{
    tut::ensure("the texture-upload sampled-binding result contains an error", result.has_value());
    return *result;
}

void ensureTextureUploadSampleBindingCode(const VulkanTextureUploadSampleBindingAcquireResult& result,
...
void acquireTextureUploadDestinationChain(FakeState& state, VulkanInstanceGeneration& owner)
{
    prepareTextureUploadDestinationState(state);
    acquireLogicalChain(state, owner);
    tut::ensure("the texture-upload-destination fixture succeeds",
                !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)));
}

void acquireTextureUploadSourceChain(FakeState& state, VulkanInstanceGeneration& owner)
{
    acquireTextureUploadDestinationChain(state, owner);
    tut::ensure("the texture-upload-source fixture succeeds",
                !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)));
}

void acquireTextureUploadTransferChain(FakeState& state, VulkanInstanceGeneration& owner)
{
    acquireTextureUploadSourceChain(state, owner);
    tut::ensure("the texture-upload-transfer fixture succeeds",
                !owner.acquireTextureUploadTransferGeneration(makeTextureUploadTransferRequest(state, owner)));
}

void acquireResidentTextureUploadDestination(FakeState& state, VulkanInstanceGeneration& owner)
{
    acquireTextureUploadTransferChain(state, owner);
    const auto  result      = owner.executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, owner));
    const auto* disposition = std::get_if<VulkanTextureUploadTransferDisposition>(&result);
    tut::ensure("the resident texture fixture completes one upload",
                disposition && *disposition == VulkanTextureUploadTransferDisposition::Complete &&
                    owner.textureUploadDestinationIsResident());
}

void acquireTextureUploadSampleBindingChain(FakeState& state, VulkanInstanceGeneration& owner)
{
    acquireResidentTextureUploadDestination(state, owner);
    tut::ensure("the texture sampled-binding fixture succeeds",
                !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)));
}
...
bool textureUploadSampleBindingMatches(const FakeState& state, const VulkanInstanceGeneration& owner) noexcept
{
    ...
    return owner.hasTextureUploadSampleBindingGeneration() &&
           owner.textureUploadSampleBindingSamplerResourceHandle() == description.mSampler.mHandle &&
           owner.textureUploadSampleBindingDestinationResourceHandle() == vulkanTextureUploadDestinationDescription().mHandle &&
           owner.textureUploadSampleBindingExpectedRevision() == LLRenderContract::TEXTURE_UPLOAD_REVISION &&
           owner.textureUploadSampleBindingResidentRevision() == LLRenderContract::TEXTURE_UPLOAD_REVISION &&
           owner.textureUploadSampleBindingResidentContentIdentity() == owner.textureUploadDestinationResidentContentIdentity() &&
           owner.textureUploadSampleBindingResidentContentIdentity() != 0 &&
           owner.textureUploadSampleBindingDestinationImageView() == state.mTextureUploadDestinationView &&
           owner.textureUploadSampleBindingDestinationImageLayout() == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL &&
           owner.textureUploadSampleBindingDescriptorSetIndex() == 0 && owner.textureUploadSampleBindingBinding() == 0 &&
           owner.textureUploadSampleBindingSampler() == state.mTextureSampleSampler &&
           owner.textureUploadSampleBindingDescriptorSetLayout() == state.mTextureSampleDescriptorSetLayout &&
...
bool textureUploadDestinationMatches(const FakeState&                                 state,
                                     const VulkanInstanceGeneration&                  owner,
                                     const VulkanTextureUploadDestinationDescription& description) noexcept
{
    const VkExtent3D                 resident_extent     = owner.textureUploadDestinationResidentExtent();
    const LLRenderContract::Extent2D logical_extent      = owner.textureUploadDestinationLogicalExtent();
    const VkImageFormatProperties    format_limits       = owner.textureUploadDestinationImageFormatProperties();
    const VkMemoryRequirements       memory_requirements = owner.textureUploadDestinationMemoryRequirements();
    const VkImageSubresourceRange    view_range          = owner.textureUploadDestinationViewRange();
    const VkFormatFeatureFlags       reported_format_features =
        state.mTextureFormatProperties.optimalTilingFeatures |
        (!state.mSwapchainFormats.empty() && state.mSwapchainFormats.front().format == VK_FORMAT_R8G8B8A8_UNORM
             ? state.mSwapchainFormatProperties.optimalTilingFeatures
             : 0);
    return owner.hasTextureUploadDestinationGeneration() && owner.textureUploadDestinationResourceHandle() == description.mHandle &&
           owner.textureUploadDestinationExpectedRevision() == description.mExpectedRevision &&
           ...
           owner.textureUploadDestinationResidentDiscard() == description.mResidentDiscard &&
           owner.textureUploadDestinationPixelFormat() == description.mFormat &&
           owner.textureUploadDestinationInitialState() == description.mInitialState && owner.textureUploadDestinationFlags() == 0 &&
           owner.textureUploadDestinationImageType() == VK_IMAGE_TYPE_2D &&
           owner.textureUploadDestinationFormat() == VK_FORMAT_R8G8B8A8_UNORM &&
           owner.textureUploadDestinationMipLevels() == description.mMipLevels &&
           owner.textureUploadDestinationArrayLayers() == description.mArrayLayers &&
           owner.textureUploadDestinationSamples() == VK_SAMPLE_COUNT_1_BIT &&
           owner.textureUploadDestinationTiling() == VK_IMAGE_TILING_OPTIMAL &&
           owner.textureUploadDestinationUsage() ==
               (VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT) &&
           owner.textureUploadDestinationSharingMode() == VK_SHARING_MODE_EXCLUSIVE &&
           owner.textureUploadDestinationInitialLayout() == VK_IMAGE_LAYOUT_UNDEFINED &&
           owner.textureUploadDestinationFormatFeatures() == reported_format_features &&
           format_limits.maxExtent.width == state.mTextureImageFormatProperties.maxExtent.width &&
           format_limits.maxExtent.height == state.mTextureImageFormatProperties.maxExtent.height &&
           format_limits.maxExtent.depth == state.mTextureImageFormatProperties.maxExtent.depth &&
           format_limits.maxMipLevels == state.mTextureImageFormatProperties.maxMipLevels &&
           format_limits.maxArrayLayers == state.mTextureImageFormatProperties.maxArrayLayers &&
           format_limits.sampleCounts == state.mTextureImageFormatProperties.sampleCounts &&
           format_limits.maxResourceSize == state.mTextureImageFormatProperties.maxResourceSize &&
           owner.textureUploadDestinationImage() == state.mTextureUploadDestinationImage &&
           owner.textureUploadDestinationMemory() == state.mTextureUploadDestinationMemory &&
           memory_requirements.size == state.mTextureImageMemoryRequirements.size &&
           memory_requirements.alignment == state.mTextureImageMemoryRequirements.alignment &&
           memory_requirements.memoryTypeBits == state.mTextureImageMemoryRequirements.memoryTypeBits &&
           owner.textureUploadDestinationAllocationSize() == state.mTextureImageMemoryRequirements.size &&
           owner.textureUploadDestinationAllocationAlignment() == state.mTextureImageMemoryRequirements.alignment &&
           owner.textureUploadDestinationCompatibleMemoryTypeBits() == state.mTextureImageMemoryRequirements.memoryTypeBits &&
           owner.textureUploadDestinationMemoryTypeIndex() == 1 &&
           owner.textureUploadDestinationMemoryPropertyFlags() == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT &&
           owner.textureUploadDestinationIsDeviceLocal() &&
           owner.textureUploadDestinationPrefersDedicatedAllocation() == (state.mTexturePrefersDedicatedAllocation != VK_FALSE) &&
           owner.textureUploadDestinationRequiresDedicatedAllocation() == (state.mTextureRequiresDedicatedAllocation != VK_FALSE) &&
           owner.textureUploadDestinationImageView() == state.mTextureUploadDestinationView &&
           owner.textureUploadDestinationImageViewType() == VK_IMAGE_VIEW_TYPE_2D && view_range.aspectMask == VK_IMAGE_ASPECT_COLOR_BIT &&
...
bool textureUploadSourceMatches(const FakeState&                            state,
                                const VulkanInstanceGeneration&             owner,
                                const VulkanTextureUploadSourceDescription& description) noexcept
{
    const LLRenderContract::Extent2D extent = owner.textureUploadSourceResidentExtent();
    return owner.hasTextureUploadSourceGeneration() && owner.textureUploadSourceResourceHandle() == description.mHandle &&
           owner.textureUploadSourceExpectedRevision() == description.mExpectedRevision &&
           ...
           owner.textureUploadSourcePixelFormat() == LLRenderContract::PixelFormat::RGBA8Unorm &&
           owner.textureUploadSourceRowPitch() == LLRenderContract::TEXTURE_UPLOAD_ROW_PITCH &&
           owner.textureUploadSourceRowOrigin() == LLRenderContract::RowOrigin::TopLeft &&
           owner.textureUploadSourceContentIdentity() == LLRenderContract::stableByteContentIdentity(description.mBytes) &&
           owner.textureUploadSourceFlags() == 0 && owner.textureUploadSourceUsage() == VK_BUFFER_USAGE_TRANSFER_SRC_BIT &&
           owner.textureUploadSourceSharingMode() == VK_SHARING_MODE_EXCLUSIVE &&
           owner.textureUploadSourceBuffer() == state.mTextureUploadSourceBuffer &&
           owner.textureUploadSourceMemory() == state.mTextureUploadSourceMemory &&
           owner.textureUploadSourceByteCount() == VULKAN_TEXTURE_UPLOAD_SOURCE_BYTE_COUNT &&
           owner.textureUploadSourceAllocationSize() == state.mTextureUploadSourceMemoryRequirements.size &&
           owner.textureUploadSourceMemoryTypeIndex() == 0 &&
           owner.textureUploadSourceMemoryPropertyFlags() == (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) &&
           owner.textureUploadSourceIsCoherent() && state.mTextureUploadSourceMappedStorage == description.mBytes;
}
...
struct TextureLogicalAbaContext
{
    VulkanInstanceGeneration* mOwner       = nullptr;
    std::size_t               mOwnerChecks = 0;
    bool                      mReset       = false;
...
bool textureLogicalAbaOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureLogicalAbaContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
...
bool textureLogicalAbaWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    const auto* context = static_cast<const TextureLogicalAbaContext*>(userdata);
    return context && context->mOwner && native_window_generation == context->mOwner->nativeWindowGeneration();
}

struct TexturePublicationContext
{
    FakeState*                mState             = nullptr;
    VulkanInstanceGeneration* mOwner             = nullptr;
    VkImage                   mReplacementImage  = VK_NULL_HANDLE;
    VkDeviceMemory            mReplacementMemory = VK_NULL_HANDLE;
    VkImageView               mReplacementView   = VK_NULL_HANDLE;
    std::size_t               mOwnerChecks       = 0;
    std::size_t               mWindowChecks      = 0;
    bool                      mAttempted         = false;
    bool                      mPublished         = false;
...
bool texturePublicationOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TexturePublicationContext*>(userdata);
    if (!context || !context->mState || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
    ...
    {
        context->mAttempted                              = true;
        context->mState->mTextureUploadDestinationImage  = context->mReplacementImage;
        context->mState->mTextureUploadDestinationMemory = context->mReplacementMemory;
        context->mState->mTextureUploadDestinationView   = context->mReplacementView;
        const VulkanTextureUploadDestinationRequest request{ context->mOwner->nativeWindowGeneration(),
                                                             vulkanTextureUploadDestinationDescription(),
                                                             { context->mOwner, exactMutationOwnerIsCurrent },
                                                             { context->mOwner, exactMutationWindowIsCurrent } };
        context->mPublished = !context->mOwner->acquireTextureUploadDestinationGeneration(request);
    }
    return true;
}

bool texturePublicationWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TexturePublicationContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
struct TextureNativeGuardContext
{
    VulkanInstanceGeneration*                       mOwner               = nullptr;
    std::size_t                                     mOwnerChecks         = 0;
    std::size_t                                     mWindowChecks        = 0;
    ...
    std::optional<VulkanInstanceGeneration>         mMoveDestination;
    std::optional<VulkanSwapchainChainRebuildError> mRebuildError;
    bool                                            mDirectResetSucceeded  = false;
    bool                                            mLogicalResetSucceeded = false;
    bool                                            mSurfaceResetSucceeded = false;
    bool                                            mFullResetSucceeded    = false;
};

bool textureGuardRebuildOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
...
bool textureGuardRebuildWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
bool textureNativeGuardOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
    ...
    {
        context->mMoveDestination.emplace(std::move(*context->mOwner));
        const VulkanSwapchainChainRebuildRequest rebuild_request{ context->mOwner->nativeWindowGeneration(),
                                                                  VkExtent2D{ 800, 600 },
                                                                  { context, textureGuardRebuildOwnerIsCurrent },
                                                                  { context, textureGuardRebuildWindowIsCurrent } };
        const VulkanSwapchainChainRebuildResult  rebuild_result = context->mOwner->rebuildSwapchainChain(rebuild_request);
        if (const auto* error = std::get_if<VulkanSwapchainChainRebuildError>(&rebuild_result))
        {
            context->mRebuildError = *error;
        }
        context->mDirectResetSucceeded  = context->mOwner->resetTextureUploadDestinationGeneration();
        context->mLogicalResetSucceeded = context->mOwner->resetLogicalDeviceGeneration();
        context->mSurfaceResetSucceeded = context->mOwner->resetSurfaceGeneration();
        context->mFullResetSucceeded    = context->mOwner->reset();
        return false;
    }
    return true;
}

bool textureNativeGuardWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
struct TextureSourceDestinationAbaContext
{
    FakeState*                mState       = nullptr;
    VulkanInstanceGeneration* mOwner       = nullptr;
    std::size_t               mOwnerChecks = 0;
...
bool textureSourceDestinationAbaOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSourceDestinationAbaContext*>(userdata);
    if (!context || !context->mState || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
    ...
    if (context->mOwnerChecks == 1)
    {
        ...
        if (context->mReset)
        {
            const VulkanTextureUploadDestinationRequest request{ context->mOwner->nativeWindowGeneration(),
                                                                 ...
                                                                 { context->mOwner, exactMutationOwnerIsCurrent },
                                                                 { context->mOwner, exactMutationWindowIsCurrent } };
            context->mReacquired = !context->mOwner->acquireTextureUploadDestinationGeneration(request);
        }
    }
    return true;
}

bool textureSourceDestinationAbaWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    const auto* context = static_cast<const TextureSourceDestinationAbaContext*>(userdata);
    return context && context->mOwner && native_window_generation == context->mOwner->nativeWindowGeneration();
}

struct TextureSourcePublicationContext
{
    FakeState*                           mState = nullptr;
    VulkanInstanceGeneration*            mOwner = nullptr;
...
bool textureSourcePublicationOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSourcePublicationContext*>(userdata);
    if (!context || !context->mState || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
    ...
    {
        context->mAttempted                         = true;
        context->mState->mTextureUploadSourceBuffer = context->mReplacementBuffer;
        context->mState->mTextureUploadSourceMemory = context->mReplacementMemory;
        const VulkanTextureUploadSourceRequest request{ context->mOwner->nativeWindowGeneration(),
                                                        context->mDescription,
                                                        { context->mOwner, exactMutationOwnerIsCurrent },
                                                        { context->mOwner, exactMutationWindowIsCurrent } };
        context->mPublished = !context->mOwner->acquireTextureUploadSourceGeneration(request);
    }
    return true;
}

bool textureSourcePublicationWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureSourcePublicationContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
struct TextureSourceNativeGuardContext
{
    VulkanInstanceGeneration*                       mOwner               = nullptr;
    std::size_t                                     mOwnerChecks         = 0;
    std::size_t                                     mWindowChecks        = 0;
...
bool textureSourceGuardRebuildOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSourceNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
...
bool textureSourceGuardRebuildWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureSourceNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
bool textureSourceNativeGuardOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSourceNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
    ...
    if (context->mOwnerChecks == 2)
    {
        ...
        const VulkanSwapchainChainRebuildRequest rebuild_request{ context->mOwner->nativeWindowGeneration(),
                                                                  VkExtent2D{ 800, 600 },
                                                                  { context, textureSourceGuardRebuildOwnerIsCurrent },
                                                                  { context, textureSourceGuardRebuildWindowIsCurrent } };
        const VulkanSwapchainChainRebuildResult  rebuild_result = context->mOwner->rebuildSwapchainChain(rebuild_request);
        if (const auto* error = std::get_if<VulkanSwapchainChainRebuildError>(&rebuild_result))
        {
            context->mRebuildError = *error;
        }
        context->mSourceResetSucceeded      = context->mOwner->resetTextureUploadSourceGeneration();
        context->mDestinationResetSucceeded = context->mOwner->resetTextureUploadDestinationGeneration();
        context->mLogicalResetSucceeded     = context->mOwner->resetLogicalDeviceGeneration();
        context->mSurfaceResetSucceeded     = context->mOwner->resetSurfaceGeneration();
        context->mFullResetSucceeded        = context->mOwner->reset();
        return false;
...
bool textureSourceNativeGuardWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureSourceNativeGuardContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration())
    {
...
struct TextureSampleBindingNestedContext
{
    VulkanInstanceGeneration* mOwner           = nullptr;
    std::size_t               mOwnerChecks     = 0;
...
bool textureSampleBindingNestedOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSampleBindingNestedContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
    ...
    if (context->mOwnerChecks == 2 && !context->mNestedAttempted)
    {
        ...
        const VulkanTextureUploadSampleBindingRequest nested_request{ context->mOwner->nativeWindowGeneration(),
                                                                      vulkanTextureUploadDestinationDescription(),
                                                                      vulkanTextureUploadSampleBindingDescription(),
                                                                      { context->mOwner, exactMutationOwnerIsCurrent },
                                                                      { context->mOwner, exactMutationWindowIsCurrent } };
        context->mNestedPublished = !context->mOwner->acquireTextureUploadSampleBindingGeneration(nested_request);
    }
    return true;
}

bool textureSampleBindingNestedWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    const auto* context = static_cast<const TextureSampleBindingNestedContext*>(userdata);
    return context && context->mOwner && native_window_generation == context->mOwner->nativeWindowGeneration();
}

struct TextureSampleBindingDestinationAbaContext
{
    FakeState*                mState              = nullptr;
    VulkanInstanceGeneration* mOwner              = nullptr;
    std::size_t               mOwnerChecks        = 0;
    bool                      mReset              = false;
    bool                      mReacquiredResident = false;
};

bool textureSampleBindingDestinationAbaOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSampleBindingDestinationAbaContext*>(userdata);
    if (!context || !context->mState || !context->mOwner || context->mOwner != &generation)
    {
        return false;
    }
    ...
    const VulkanTextureUploadTransferOperationRequest operation_request{ context->mOwner->nativeWindowGeneration(),
                                                                         ...
                                                                         owner_check,
                                                                         window_check };
    if (context->mOwner->acquireTextureUploadDestinationGeneration(destination_request) ||
    ...
    const auto  result           = context->mOwner->executeTextureUploadTransfer(operation_request);
    const auto* disposition      = std::get_if<VulkanTextureUploadTransferDisposition>(&result);
    context->mReacquiredResident = disposition && *disposition == VulkanTextureUploadTransferDisposition::Complete &&
                                   context->mOwner->textureUploadDestinationIsResident();
    return true;
}

bool textureSampleBindingDestinationAbaWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    const auto* context = static_cast<const TextureSampleBindingDestinationAbaContext*>(userdata);
    return context && context->mOwner && native_window_generation == context->mOwner->nativeWindowGeneration();
}

struct TextureSampleBindingRequestMutationContext
{
    VulkanInstanceGeneration*                mOwner               = nullptr;
    VulkanTextureUploadSampleBindingRequest* mRequest             = nullptr;
    bool                                     mMutateOwnerCallback = false;
    bool                                     mMutated             = false;
    std::size_t                              mOwnerChecks         = 0;
    std::size_t                              mWindowChecks        = 0;
};

bool textureSampleBindingRequestMutationOwnerIsCurrent(void* userdata, const VulkanInstanceGeneration& generation) noexcept
{
    auto* context = static_cast<TextureSampleBindingRequestMutationContext*>(userdata);
    if (!context || !context->mOwner || context->mOwner != &generation || !context->mRequest)
    {
        return false;
    }
    ++context->mOwnerChecks;
    if (context->mMutateOwnerCallback && !context->mMutated)
    {
        context->mRequest->mInstanceOwnerCheck.mIsCurrent = nullptr;
        context->mMutated                                 = true;
    }
    return true;
}

bool textureSampleBindingRequestMutationWindowIsCurrent(void* userdata, std::uint64_t native_window_generation) noexcept
{
    auto* context = static_cast<TextureSampleBindingRequestMutationContext*>(userdata);
    if (!context || !context->mOwner || native_window_generation != context->mOwner->nativeWindowGeneration() || !context->mRequest)
    {
        return false;
    }
    ++context->mWindowChecks;
    if (!context->mMutateOwnerCallback && !context->mMutated)
    {
        context->mRequest->mWindowGenerationCheck.mIsCurrent = nullptr;
        context->mMutated                                    = true;
    }
    return true;
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<125>()
{
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().hasTextureUploadDestinationGeneration()));
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().textureUploadDestinationImage()));
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().textureUploadDestinationMemory()));
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().textureUploadDestinationImageView()));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().acquireTextureUploadDestinationGeneration(
        std::declval<const VulkanTextureUploadDestinationRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().resetTextureUploadDestinationGeneration()));

    FakeState                     state;
    ScopedFakeState               scope(state);
    VulkanInstanceGeneration      owner        = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    const VkImageSubresourceRange inert_range  = owner.textureUploadDestinationViewRange();
    const VkMemoryRequirements    inert_memory = owner.textureUploadDestinationMemoryRequirements();
    ensure(
        "an absent texture image owner exposes only inert metadata",
        !owner.hasTextureUploadDestinationGeneration() && !owner.textureUploadDestinationResourceHandle() &&
            owner.textureUploadDestinationExpectedRevision() == 0 && owner.textureUploadDestinationResidentExtent().width == 0 &&
            owner.textureUploadDestinationResidentExtent().height == 0 && owner.textureUploadDestinationResidentExtent().depth == 0 &&
            owner.textureUploadDestinationLogicalExtent().mWidth == 0 && owner.textureUploadDestinationLogicalExtent().mHeight == 0 &&
            owner.textureUploadDestinationResidentDiscard() == 0 &&
            owner.textureUploadDestinationPixelFormat() == LLRenderContract::PixelFormat::RGBA8Unorm &&
            owner.textureUploadDestinationInitialState() == LLRenderContract::ImageState::Undefined &&
            owner.textureUploadDestinationFlags() == 0 && owner.textureUploadDestinationImageType() == VK_IMAGE_TYPE_MAX_ENUM &&
            owner.textureUploadDestinationFormat() == VK_FORMAT_UNDEFINED && owner.textureUploadDestinationMipLevels() == 0 &&
            owner.textureUploadDestinationArrayLayers() == 0 &&
            owner.textureUploadDestinationSamples() == VK_SAMPLE_COUNT_FLAG_BITS_MAX_ENUM &&
            owner.textureUploadDestinationTiling() == VK_IMAGE_TILING_MAX_ENUM && owner.textureUploadDestinationUsage() == 0 &&
            owner.textureUploadDestinationSharingMode() == VK_SHARING_MODE_MAX_ENUM &&
            owner.textureUploadDestinationInitialLayout() == VK_IMAGE_LAYOUT_MAX_ENUM &&
            owner.textureUploadDestinationFormatFeatures() == 0 && owner.textureUploadDestinationImage() == VK_NULL_HANDLE &&
            owner.textureUploadDestinationMemory() == VK_NULL_HANDLE && inert_memory.size == 0 && inert_memory.alignment == 0 &&
            inert_memory.memoryTypeBits == 0 && owner.textureUploadDestinationAllocationSize() == 0 &&
            owner.textureUploadDestinationAllocationAlignment() == 0 && owner.textureUploadDestinationCompatibleMemoryTypeBits() == 0 &&
            owner.textureUploadDestinationMemoryTypeIndex() == 0 && owner.textureUploadDestinationMemoryPropertyFlags() == 0 &&
            !owner.textureUploadDestinationIsDeviceLocal() && !owner.textureUploadDestinationPrefersDedicatedAllocation() &&
            !owner.textureUploadDestinationRequiresDedicatedAllocation() && owner.textureUploadDestinationImageView() == VK_NULL_HANDLE &&
            owner.textureUploadDestinationImageViewType() == VK_IMAGE_VIEW_TYPE_MAX_ENUM && inert_range.aspectMask == 0 &&
            inert_range.baseMipLevel == 0 && inert_range.levelCount == 0 && inert_range.baseArrayLayer == 0 && inert_range.layerCount == 0);
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)),
                                       VulkanTextureUploadDestinationAcquireCode::SurfaceNotLive);

    prepareTextureUploadDestinationState(state);
    ensure("the surface preflight fixture succeeds", !owner.acquireSurfaceGeneration(makeSurfaceRequest(state, owner)));
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)),
                                       VulkanTextureUploadDestinationAcquireCode::PresentationDeviceNotLive);
    ensure("the presentation-device preflight fixture succeeds",
           !owner.acquirePresentationDeviceGeneration(makePresentationDeviceRequest(state, owner)));
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)),
                                       VulkanTextureUploadDestinationAcquireCode::LogicalDeviceNotLive);
    ensure("the logical-device preflight fixture succeeds", !owner.acquireLogicalDeviceGeneration(makeLogicalDeviceRequest(state, owner)));
    VulkanTextureUploadDestinationRequest request = makeTextureUploadDestinationRequest(state, owner);
    request.mInstanceOwnerCheck                   = {};
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::InvalidInstanceOwnerCheck);
    request                        = makeTextureUploadDestinationRequest(state, owner);
    request.mWindowGenerationCheck = {};
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::InvalidWindowGenerationCheck);
    request                         = makeTextureUploadDestinationRequest(state, owner);
    request.mNativeWindowGeneration = 0;
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::InvalidNativeWindowGeneration);
    request                         = makeTextureUploadDestinationRequest(state, owner);
    request.mNativeWindowGeneration = 41;
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::NativeWindowGenerationMismatch);
    request                     = makeTextureUploadDestinationRequest(state, owner);
    state.mInstanceOwnerCurrent = false;
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::StaleInstanceOwner);
    state.mInstanceOwnerCurrent = true;
    state.mSurfaceWindowCurrent = false;
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::StaleWindowGeneration);
    state.mSurfaceWindowCurrent = true;

    request                      = makeTextureUploadDestinationRequest(state, owner);
    request.mDescription.mHandle = {};
    const auto  invalid_result   = owner.acquireTextureUploadDestinationGeneration(request);
    const auto& invalid_error    = requireTextureUploadDestinationError(invalid_result);
    ensure("the canonical description fails in the core before native image work",
           invalid_error.mCode == VulkanTextureUploadDestinationAcquireCode::ResolutionFailure && invalid_error.mResolutionError &&
               invalid_error.mResolutionError->mCode == VulkanTextureUploadDestinationResolutionCode::InvalidDescription &&
               state.mTextureFormatPropertiesCalls == 0 && state.mTextureImageFormatPropertiesCalls == 0 &&
               state.mCreateTextureImageCalls == 0 && state.mAllocateTextureMemoryCalls == 0 && state.mCreateTextureImageViewCalls == 0);

    const VulkanTextureUploadDestinationDescription description = vulkanTextureUploadDestinationDescription();
    request                                                     = makeTextureUploadDestinationRequest(state, owner, description);
    ensure("one canonical texture image owner publishes", !owner.acquireTextureUploadDestinationGeneration(request));
    ensure("the aggregate exposes the exact image, allocation, and view metadata",
           textureUploadDestinationMatches(state, owner, description));
    ensure("publication queried and created the canonical optimal-tiled image",
           state.mTextureFormatPropertiesCalls == 1 && state.mTextureFormatPropertiesDevice == state.mPhysicalDevice &&
               state.mTextureFormatPropertiesFormat == VK_FORMAT_R8G8B8A8_UNORM && state.mTextureImageFormatPropertiesCalls == 1 &&
               state.mTextureImageFormatPropertiesDevice == state.mPhysicalDevice &&
               state.mTextureImageFormat == VK_FORMAT_R8G8B8A8_UNORM && state.mTextureImageType == VK_IMAGE_TYPE_2D &&
               state.mTextureImageTiling == VK_IMAGE_TILING_OPTIMAL && state.mTextureImageFlags == 0 &&
               state.mTextureImageUsage ==
                   (VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT) &&
               state.mCreateTextureImageCalls == 1 && state.mTextureImageCreateInfo.sType == VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO &&
               ...
               state.mTextureImageCreateInfo.extent.depth == 1 && state.mTextureImageCreateInfo.mipLevels == description.mMipLevels &&
               ...
               state.mCreateTextureImageAllocationCallbacks == std::vector<const VkAllocationCallbacks*>{ nullptr });
    ensure("publication uses one formally dedicated device-local allocation",
           state.mGetTextureImageMemoryRequirementsCalls == 1 &&
               state.mTextureMemoryRequirementsImage == state.mTextureUploadDestinationImage && state.mAllocateTextureMemoryCalls == 1 &&
               state.mTextureMemoryAllocateInfo.sType == VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO &&
               state.mTextureMemoryAllocateInfo.allocationSize == state.mTextureImageMemoryRequirements.size &&
               state.mTextureMemoryAllocateInfo.memoryTypeIndex == 1 &&
               state.mTextureDedicatedAllocateInfo.sType == VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO &&
               state.mTextureDedicatedAllocateInfo.image == state.mTextureUploadDestinationImage &&
               state.mTextureDedicatedAllocateInfo.buffer == VK_NULL_HANDLE && state.mBindTextureImageMemoryCalls == 1 &&
               state.mTextureBoundImage == state.mTextureUploadDestinationImage &&
               state.mTextureBoundMemory == state.mTextureUploadDestinationMemory && state.mTextureBindOffset == 0);
    ensure("publication creates one full-color view over every declared mip",
           state.mCreateTextureImageViewCalls == 1 && state.mTextureImageViewCreateInfo.sType == VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO &&
               state.mTextureImageViewCreateInfo.image == state.mTextureUploadDestinationImage &&
    ...
    const std::size_t owner_checks       = state.mInstanceOwnerChecks;
    const std::size_t window_checks      = state.mSurfaceWindowChecks;
    const std::size_t device_proc_calls  = state.mDeviceProcAddrCalls;
    const std::size_t create_image_calls = state.mCreateTextureImageCalls;
    const std::size_t allocate_calls     = state.mAllocateTextureMemoryCalls;
    const std::size_t create_view_calls  = state.mCreateTextureImageViewCalls;
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::TextureUploadDestinationAlreadyOwned);
    ensure("duplicate acquisition stops before callbacks, command lookup, or native work",
           state.mInstanceOwnerChecks == owner_checks && state.mSurfaceWindowChecks == window_checks &&
               state.mDeviceProcAddrCalls == device_proc_calls && state.mCreateTextureImageCalls == create_image_calls &&
               state.mAllocateTextureMemoryCalls == allocate_calls && state.mCreateTextureImageViewCalls == create_view_calls &&
               owner.reset());
    ensure("the published ownership occurrence retires once in view-image-memory order",
           state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1);
}

template<>
template<>
void render_vulkan_instance_test_object::test<126>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        ...
        const auto result = owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner));
        ensureTextureUploadDestinationCode(result, VulkanTextureUploadDestinationAcquireCode::StaleInstanceOwner);
        ensure("post-resolution staleness rolls the complete unpublished image occurrence back",
               !owner.hasTextureUploadDestinationGeneration() && state.mCreateTextureImageCalls == 1 &&
                   state.mAllocateTextureMemoryCalls == 1 && state.mBindTextureImageMemoryCalls == 1 &&
                   state.mCreateTextureImageViewCalls == 1 && state.mDestroyTextureImageViewCalls == 1 &&
                   state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1 &&
                   state.mDestroyedTextureImageViews.front() == state.mTextureUploadDestinationView &&
                   state.mDestroyedTextureImages.front() == state.mTextureUploadDestinationImage &&
                   state.mFreedTextureMemories.front() == state.mTextureUploadDestinationMemory);

        state.mFailInstanceOwnerCheck = 0;
        ensure("the exact parent can publish a second ownership occurrence with the same handle bits",
               !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)) &&
                   owner.hasTextureUploadDestinationGeneration() && state.mCreatedTextureImages.size() == 2 &&
                   state.mCreatedTextureImages[0] == state.mCreatedTextureImages[1] && state.mAllocatedTextureMemories.size() == 2 &&
                   state.mAllocatedTextureMemories[0] == state.mAllocatedTextureMemories[1] &&
                   state.mCreatedTextureImageViews.size() == 2 &&
                   state.mCreatedTextureImageViews[0] == state.mCreatedTextureImageViews[1] && owner.reset());
        ensure("each same-bit ownership occurrence retires exactly once",
               state.mDestroyedTextureImageViews.size() == 2 && state.mDestroyedTextureImages.size() == 2 &&
                   state.mFreedTextureMemories.size() == 2);
    }

    {
        ...
        const auto result =
            VulkanInstanceDetail::acquireTextureUploadDestination(owner, makeTextureUploadDestinationRequest(state, owner), failAllocation);
        ensureTextureUploadDestinationCode(result, VulkanTextureUploadDestinationAcquireCode::AllocationFailure);
        ensure("wrapper allocation failure destroys the fully resolved image candidate without publication",
               !owner.hasTextureUploadDestinationGeneration() && state.mCreateTextureImageCalls == 1 &&
                   state.mAllocateTextureMemoryCalls == 1 && state.mCreateTextureImageViewCalls == 1 &&
                   state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1);
        ensure("allocation rollback leaves the logical parent reusable",
               !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)) &&
                   owner.hasTextureUploadDestinationGeneration() && owner.reset());
    }
}

template<>
template<>
void render_vulkan_instance_test_object::test<127>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        ...
        prepareTextureUploadDestinationState(state);
        acquireLogicalChain(state, owner);
        TextureLogicalAbaContext                    context{ &owner };
        const VulkanTextureUploadDestinationRequest request{ owner.nativeWindowGeneration(),
                                                             vulkanTextureUploadDestinationDescription(),
                                                             { &context, textureLogicalAbaOwnerIsCurrent },
                                                             { &context, textureLogicalAbaWindowIsCurrent } };
        ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                           VulkanTextureUploadDestinationAcquireCode::LogicalDeviceNotLive);
        ensure("the ownership epoch rejects a same-handle logical-device ABA before image creation",
               context.mOwnerChecks == 1 && context.mReset && context.mReacquired && owner.hasLogicalDeviceGeneration() &&
                   owner.logicalDevice() == state.mDevice && !owner.hasTextureUploadDestinationGeneration() &&
                   state.mCreateTextureImageCalls == 0);
        ensure("the replacement logical generation accepts a fresh authenticated image request",
               !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)) && owner.reset());
    }

    {
        ...
        prepareTextureUploadDestinationState(state);
        acquireLogicalChain(state, owner);
        const VkImage             original_image  = state.mTextureUploadDestinationImage;
        const VkDeviceMemory      original_memory = state.mTextureUploadDestinationMemory;
        const VkImageView         original_view   = state.mTextureUploadDestinationView;
        TexturePublicationContext context{ &state, &owner, fakeHandle<VkImage>(0xf101), fakeHandle<VkDeviceMemory>(0xf102),
                                           fakeHandle<VkImageView>(0xf103) };
        const VulkanTextureUploadDestinationRequest request{ owner.nativeWindowGeneration(),
                                                             vulkanTextureUploadDestinationDescription(),
                                                             { &context, texturePublicationOwnerIsCurrent },
                                                             { &context, texturePublicationWindowIsCurrent } };
        ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                           VulkanTextureUploadDestinationAcquireCode::TextureUploadDestinationAlreadyOwned);
        ensure("a nested exact publication wins without being overwritten by the older candidate",
               context.mAttempted && context.mPublished && context.mOwnerChecks == 2 && context.mWindowChecks == 1 &&
                   textureUploadDestinationMatches(state, owner, vulkanTextureUploadDestinationDescription()) &&
                   state.mCreatedTextureImages.size() == 2 && state.mAllocatedTextureMemories.size() == 2 &&
                   state.mCreatedTextureImageViews.size() == 2);
        ensure("only the losing outer ownership occurrence rolls back",
               state.mDestroyedTextureImageViews == std::vector<VkImageView>{ original_view } &&
                   state.mDestroyedTextureImages == std::vector<VkImage>{ original_image } &&
                   state.mFreedTextureMemories == std::vector<VkDeviceMemory>{ original_memory } &&
                   owner.textureUploadDestinationImage() == context.mReplacementImage &&
                   owner.textureUploadDestinationMemory() == context.mReplacementMemory &&
                   owner.textureUploadDestinationImageView() == context.mReplacementView && owner.reset());
        ensure("the winning nested occurrence retires once at aggregate teardown",
               state.mDestroyedTextureImageViews.size() == 2 && state.mDestroyedTextureImageViews.back() == context.mReplacementView &&
                   state.mDestroyedTextureImages.size() == 2 && state.mDestroyedTextureImages.back() == context.mReplacementImage &&
                   state.mFreedTextureMemories.size() == 2 && state.mFreedTextureMemories.back() == context.mReplacementMemory);
    }
}

template<>
template<>
void render_vulkan_instance_test_object::test<128>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    prepareTextureUploadDestinationState(state);
    acquireLogicalChain(state, owner);
    TextureNativeGuardContext                   context{ &owner };
    const VulkanTextureUploadDestinationRequest request{ owner.nativeWindowGeneration(),
                                                         vulkanTextureUploadDestinationDescription(),
                                                         { &context, textureNativeGuardOwnerIsCurrent },
                                                         { &context, textureNativeGuardWindowIsCurrent } };
    ensureTextureUploadDestinationCode(owner.acquireTextureUploadDestinationGeneration(request),
                                       VulkanTextureUploadDestinationAcquireCode::StaleInstanceOwner);
    ensure("a live native image candidate blocks move, rebuild, and every direct or transitive reset",
           context.mOwnerChecks == 2 && context.mWindowChecks == 1 && context.mMoveDestination &&
               context.mMoveDestination->instance() == VK_NULL_HANDLE && context.mRebuildError &&
               context.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
               context.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight && context.mRebuildOwnerChecks == 0 &&
               context.mRebuildWindowChecks == 0 && !context.mDirectResetSucceeded && !context.mLogicalResetSucceeded &&
               !context.mSurfaceResetSucceeded && !context.mFullResetSucceeded && owner.hasSurfaceGeneration() &&
               owner.hasPresentationDeviceGeneration() && owner.hasLogicalDeviceGeneration() &&
               !owner.hasTextureUploadDestinationGeneration() && state.mDestroyDeviceCalls == 0 && state.mDestroySurfaceCalls == 0);
    ensure("stale publication rolls the guarded native occurrence back after the callback returns",
           state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1);
    ensure("parent teardown succeeds after the native-acquisition guard leaves scope", owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<129>()
{
    constexpr VkExtent2D     initial_extent{ 800, 600 };
    constexpr VkExtent2D     rebuilt_extent{ 1280, 720 };
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner         = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    state.mSwapchainFormats.front().format = VK_FORMAT_R8G8B8A8_UNORM;
    acquireTextureUploadDestinationChain(state, owner);
    state.mUploadDestinationMemoryRequirements.memoryTypeBits = 3;
    ensure("the independent upload-source sibling publishes", !owner.acquireUploadSourceGeneration(makeUploadSourceRequest(state, owner)));
    ensure("the independent upload-destination sibling publishes",
           !owner.acquireUploadDestinationGeneration(makeUploadDestinationRequest(state, owner)));
    acquireCompleteSwapchainChildren(state, owner, initial_extent);
    const auto           description        = vulkanTextureUploadDestinationDescription();
    const VkImage        image              = owner.textureUploadDestinationImage();
    const VkDeviceMemory memory             = owner.textureUploadDestinationMemory();
    const VkImageView    view               = owner.textureUploadDestinationImageView();
    const VkBuffer       upload_source      = owner.uploadSourceBuffer();
    const VkBuffer       upload_destination = owner.uploadDestinationBuffer();
    ensure("one RGBA8 capability record supports both the texture owner and swapchain",
           owner.swapchainSurfaceFormat().format == VK_FORMAT_R8G8B8A8_UNORM &&
               textureUploadDestinationMatches(state, owner, description) && state.mSwapchainFormatPropertiesCalls >= 2 &&
               state.mTextureFormatPropertiesCalls >= 2);

    ensureSwapchainChainRebuildOutcome(owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, rebuilt_extent)),
                                       VulkanSwapchainChainRebuildOutcome::Ready);
    ensure("a successful changed-extent rebuild preserves the exact device-scoped texture image occurrence",
           textureUploadDestinationMatches(state, owner, description) && owner.textureUploadDestinationImage() == image &&
               owner.textureUploadDestinationMemory() == memory && owner.textureUploadDestinationImageView() == view &&
               state.mDestroyTextureImageViewCalls == 0 && state.mDestroyTextureImageCalls == 0 && state.mFreeTextureMemoryCalls == 0);

    ensure("direct texture reset preserves the complete swapchain and both buffer-upload siblings",
           owner.resetTextureUploadDestinationGeneration() && !owner.hasTextureUploadDestinationGeneration() &&
               owner.hasLogicalDeviceGeneration() && owner.hasSwapchainConfigurationGeneration() && owner.hasSwapchainGeneration() &&
               owner.hasSwapchainImagesGeneration() && owner.hasSwapchainPresentationTargetGeneration() &&
               owner.hasSwapchainPresentationPipelineGeneration() && owner.hasSwapchainReadbackGeneration() &&
               owner.hasSwapchainFrameSlotGeneration() && owner.uploadSourceBuffer() == upload_source &&
               owner.uploadDestinationBuffer() == upload_destination && state.mDestroyTextureImageViewCalls == 1 &&
               state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1);
    ensure("the reset slot reacquires a second ownership occurrence with the same handle bits",
           !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)) &&
               owner.textureUploadDestinationImage() == image && owner.textureUploadDestinationMemory() == memory &&
               owner.textureUploadDestinationImageView() == view && state.mCreatedTextureImages.size() == 2 &&
               state.mAllocatedTextureMemories.size() == 2 && state.mCreatedTextureImageViews.size() == 2);

    state.mSwapchainCreateResult = VK_ERROR_OUT_OF_HOST_MEMORY;
    const auto  failed_result    = owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1440, 810 }));
    const auto& failed           = requireSwapchainChainRebuildError(failed_result);
    ensure("the forced rebuild failure is reported at the replacement swapchain",
           failed.mCode == VulkanSwapchainChainRebuildCode::ChildFailure && failed.mPhase == VulkanSwapchainChainRebuildPhase::Swapchain &&
               std::holds_alternative<VulkanSwapchainAcquireError>(failed.mChildError));
    ensure("failed rebuild rollback preserves the exact reacquired texture occurrence without retirement",
           textureUploadDestinationMatches(state, owner, description) && owner.textureUploadDestinationImage() == image &&
               owner.textureUploadDestinationMemory() == memory && owner.textureUploadDestinationImageView() == view &&
               state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1);

    VulkanInstanceGeneration      moved(std::move(owner));
    const VkImageSubresourceRange moved_from_range = owner.textureUploadDestinationViewRange();
    ensure("aggregate move transfers the texture image owner and empties every moved-from handle",
           owner.instance() == VK_NULL_HANDLE && !owner.hasTextureUploadDestinationGeneration() &&
               !owner.textureUploadDestinationResourceHandle() && owner.textureUploadDestinationImage() == VK_NULL_HANDLE &&
               owner.textureUploadDestinationMemory() == VK_NULL_HANDLE && owner.textureUploadDestinationImageView() == VK_NULL_HANDLE &&
               moved_from_range.aspectMask == 0 && moved_from_range.levelCount == 0 &&
               textureUploadDestinationMatches(state, moved, description));

    state.mDeviceDestroyOwner  = &moved;
    state.mSurfaceDestroyOwner = &moved;
    state.mEvents.clear();
    ensure("surface reset cascades through the texture owner and logical device", moved.resetSurfaceGeneration());
    const auto view_destroy    = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    const auto image_destroy   = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImage);
    const auto memory_free     = std::find(state.mEvents.begin(), state.mEvents.end(), Event::FreeTextureMemory);
    const auto device_destroy  = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyDevice);
    const auto surface_destroy = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroySurface);
    ensure("texture view, image, and dedicated memory retire in order before device and surface parents",
           view_destroy != state.mEvents.end() && image_destroy != state.mEvents.end() && memory_free != state.mEvents.end() &&
               device_destroy != state.mEvents.end() && surface_destroy != state.mEvents.end() && view_destroy < image_destroy &&
               image_destroy < memory_free && memory_free < device_destroy && device_destroy < surface_destroy &&
               state.mDeviceDestroyObservationMade && !state.mObservedTextureUploadDestinationAtDeviceDestroy &&
               state.mSurfaceDestroyObservationMade && !state.mObservedTextureUploadDestinationAtSurfaceDestroy &&
               state.mCreatedTextureImages.size() == 2 && state.mDestroyedTextureImages.size() == 2 &&
               state.mAllocatedTextureMemories.size() == 2 && state.mFreedTextureMemories.size() == 2 &&
               state.mCreatedTextureImageViews.size() == 2 && state.mDestroyedTextureImageViews.size() == 2 &&
               !moved.hasTextureUploadDestinationGeneration() && !moved.hasLogicalDeviceGeneration() &&
               !moved.hasPresentationDeviceGeneration() && !moved.hasSurfaceGeneration());
    ensure("the remaining instance generation resets after the surface cascade", moved.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<130>()
{
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().hasTextureUploadSourceGeneration()));
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().textureUploadSourceContentIdentity()));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().acquireTextureUploadSourceGeneration(
        std::declval<const VulkanTextureUploadSourceRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().resetTextureUploadSourceGeneration()));

    FakeState                        state;
    ScopedFakeState                  scope(state);
    VulkanInstanceGeneration         owner         = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    const LLRenderContract::Extent2D absent_extent = owner.textureUploadSourceResidentExtent();
    ensure("an absent texture upload source exposes only inert metadata",
           !owner.hasTextureUploadSourceGeneration() && !owner.textureUploadSourceResourceHandle() &&
               owner.textureUploadSourceExpectedRevision() == 0 && absent_extent.mWidth == 0 && absent_extent.mHeight == 0 &&
               owner.textureUploadSourceRowPitch() == 0 && owner.textureUploadSourceContentIdentity() == 0 &&
               owner.textureUploadSourceFlags() == VK_BUFFER_CREATE_FLAG_BITS_MAX_ENUM && owner.textureUploadSourceUsage() == 0 &&
               owner.textureUploadSourceSharingMode() == VK_SHARING_MODE_MAX_ENUM && owner.textureUploadSourceBuffer() == VK_NULL_HANDLE &&
               owner.textureUploadSourceMemory() == VK_NULL_HANDLE && owner.textureUploadSourceByteCount() == 0 &&
               owner.textureUploadSourceAllocationSize() == 0 && owner.textureUploadSourceMemoryTypeIndex() == 0 &&
               owner.textureUploadSourceMemoryPropertyFlags() == 0 && !owner.textureUploadSourceIsCoherent());

    acquireLogicalChain(state, owner);
    ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)),
                                  VulkanTextureUploadSourceAcquireCode::TextureUploadDestinationNotLive);
    ensure("source acquisition performs no native work before its exact image destination exists", state.mCreateBufferCalls == 0);

    prepareTextureUploadDestinationState(state);
    ensure("the destination dependency publishes",
           !owner.acquireTextureUploadDestinationGeneration(makeTextureUploadDestinationRequest(state, owner)));
    const VulkanTextureUploadSourceDescription description = makeTextureUploadSourceDescription();
    ensure("one exact immutable texture source publishes",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner, description)));
    ensure("the aggregate exposes the exact source provenance and native allocation",
           textureUploadSourceMatches(state, owner, description));
    ensure("publication owns one exclusive 144-byte transfer source without recording GPU work",
           state.mCreateBufferCalls == 1 && state.mTextureUploadSourceBufferCreateInfo.sType == VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO &&
               state.mTextureUploadSourceBufferCreateInfo.size == VULKAN_TEXTURE_UPLOAD_SOURCE_BYTE_COUNT &&
               state.mTextureUploadSourceBufferCreateInfo.usage == VK_BUFFER_USAGE_TRANSFER_SRC_BIT &&
               state.mTextureUploadSourceBufferCreateInfo.sharingMode == VK_SHARING_MODE_EXCLUSIVE &&
               state.mTextureUploadSourceMemoryAllocateInfo.allocationSize == state.mTextureUploadSourceMemoryRequirements.size &&
               state.mTextureUploadSourceBoundBuffer == state.mTextureUploadSourceBuffer &&
               state.mTextureUploadSourceBoundMemory == state.mTextureUploadSourceMemory && state.mTextureUploadSourceBindOffset == 0 &&
               state.mMapMemoryCalls == 1 && state.mUnmapMemoryCalls == 1 &&
               state.mTextureUploadSourceMappedStorage == description.mBytes && state.mQueueSubmitCalls == 0);

    const std::size_t owner_checks      = state.mInstanceOwnerChecks;
    const std::size_t window_checks     = state.mSurfaceWindowChecks;
    const std::size_t create_calls      = state.mCreateBufferCalls;
    const std::size_t device_proc_calls = state.mDeviceProcAddrCalls;
    ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner, description)),
                                  VulkanTextureUploadSourceAcquireCode::TextureUploadSourceAlreadyOwned);
    ensure("duplicate acquisition stops before callbacks, command lookup, or native work",
           state.mInstanceOwnerChecks == owner_checks && state.mSurfaceWindowChecks == window_checks &&
               state.mCreateBufferCalls == create_calls && state.mDeviceProcAddrCalls == device_proc_calls);

    ensure("direct source reset preserves its exact image destination",
           owner.resetTextureUploadSourceGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadDestinationGeneration() &&
               owner.textureUploadDestinationImage() == state.mTextureUploadDestinationImage && state.mDestroyBufferCalls == 1 &&
               state.mFreeMemoryCalls == 1);
    ensure("the reset source slot accepts a second same-bit ownership occurrence",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner, description)) &&
               owner.textureUploadSourceBuffer() == state.mTextureUploadSourceBuffer && state.mCreatedBuffers.size() == 2);
    ensure("destination reset cascades through its source before retiring the image", owner.resetTextureUploadDestinationGeneration());
    ensure("the dependency cascade leaves the logical parent live and retires both exact occurrences once",
           !owner.hasTextureUploadSourceGeneration() && !owner.hasTextureUploadDestinationGeneration() &&
               owner.hasLogicalDeviceGeneration() && state.mDestroyBufferCalls == 2 && state.mFreeMemoryCalls == 3 &&
               state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1 &&
               owner.reset());
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<131>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        ...
        VulkanTextureUploadSourceRequest request = makeTextureUploadSourceRequest(state, owner);
        request.mInstanceOwnerCheck.mIsCurrent   = nullptr;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::InvalidInstanceOwnerCheck);
        request                                   = makeTextureUploadSourceRequest(state, owner);
        request.mWindowGenerationCheck.mIsCurrent = nullptr;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::InvalidWindowGenerationCheck);
        request                         = makeTextureUploadSourceRequest(state, owner);
        request.mNativeWindowGeneration = 0;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::InvalidNativeWindowGeneration);
        request                         = makeTextureUploadSourceRequest(state, owner);
        request.mNativeWindowGeneration = 43;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::NativeWindowGenerationMismatch);
        request                     = makeTextureUploadSourceRequest(state, owner);
        state.mInstanceOwnerCurrent = false;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::StaleInstanceOwner);
        state.mInstanceOwnerCurrent = true;
        state.mSurfaceWindowCurrent = false;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::StaleWindowGeneration);
        state.mSurfaceWindowCurrent = true;

        request                      = makeTextureUploadSourceRequest(state, owner);
        request.mDescription.mHandle = LLRenderContract::StreamingUploadHandles{}.mOldImage;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::TextureUploadDestinationNotLive);
        request = makeTextureUploadSourceRequest(state, owner);
        ++request.mDescription.mExpectedRevision;
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::TextureUploadDestinationNotLive);
        ensure("invalid and mismatched preflight requests perform no source allocation", state.mCreateBufferCalls == 0);

        state.mMissing = MissingCommand::CreateBuffer;
        const VulkanTextureUploadSourceAcquireResult missing_result =
            owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner));
        const VulkanTextureUploadSourceAcquireError& missing_error = requireTextureUploadSourceError(missing_result);
        ensure("the aggregate preserves the typed standalone resolution failure",
               missing_error.mCode == VulkanTextureUploadSourceAcquireCode::ResolutionFailure && missing_error.mResolutionError &&
                   missing_error.mResolutionError->mCode == VulkanTextureUploadSourceResolutionCode::MissingRequiredCommand &&
                   missing_error.mResolutionError->mCommand == VulkanTextureUploadSourceCommand::CreateBuffer &&
                   state.mCreateBufferCalls == 0);
        state.mMissing = MissingCommand::None;

        VulkanTextureUploadSourceDescription exact = makeTextureUploadSourceDescription();
        for (std::size_t index = 0; index < exact.mBytes.size(); ++index)
        {
            exact.mBytes[index] ^= static_cast<std::uint8_t>(0x5aU + index);
        }
        ensure("the aggregate retains and publishes all caller-owned source bytes",
               !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner, exact)) &&
                   textureUploadSourceMatches(state, owner, exact) && owner.reset());
    }

    {
        ...
        acquireTextureUploadDestinationChain(state, owner);
        state.mFailInstanceOwnerCheck = state.mInstanceOwnerChecks + 2;
        const auto result             = owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner));
        ensureTextureUploadSourceCode(result, VulkanTextureUploadSourceAcquireCode::StaleInstanceOwner);
        ensure("post-resolution staleness rolls the complete unpublished source occurrence back",
               !owner.hasTextureUploadSourceGeneration() && owner.hasTextureUploadDestinationGeneration() &&
                   state.mCreateBufferCalls == 1 && state.mAllocateMemoryCalls == 2 && state.mMapMemoryCalls == 1 &&
                   state.mUnmapMemoryCalls == 1 && state.mDestroyBufferCalls == 1 && state.mFreeMemoryCalls == 1 &&
                   state.mQueueSubmitCalls == 0);
        state.mFailInstanceOwnerCheck = 0;
        ensure("the exact destination remains reusable after freshness rollback",
               !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) && owner.reset());
    }

    {
        ...
        acquireTextureUploadDestinationChain(state, owner);
        const auto result =
            VulkanInstanceDetail::acquireTextureUploadSource(owner, makeTextureUploadSourceRequest(state, owner), failAllocation);
        ensureTextureUploadSourceCode(result, VulkanTextureUploadSourceAcquireCode::AllocationFailure);
        ensure("wrapper allocation failure destroys the resolved native source without publication",
               !owner.hasTextureUploadSourceGeneration() && owner.hasTextureUploadDestinationGeneration() &&
                   state.mCreateBufferCalls == 1 && state.mMapMemoryCalls == 1 && state.mUnmapMemoryCalls == 1 &&
                   state.mDestroyBufferCalls == 1 && state.mFreeMemoryCalls == 1 && state.mQueueSubmitCalls == 0);
        ensure("allocation rollback leaves the exact dependency chain reusable",
               !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) && owner.reset());
    }
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<132>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
        acquireTextureUploadDestinationChain(state, owner);
        TextureSourceDestinationAbaContext     context{ &state, &owner };
        const VulkanTextureUploadSourceRequest request{ owner.nativeWindowGeneration(),
                                                        makeTextureUploadSourceDescription(),
                                                        { &context, textureSourceDestinationAbaOwnerIsCurrent },
                                                        { &context, textureSourceDestinationAbaWindowIsCurrent } };
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::TextureUploadDestinationNotLive);
        ensure("the dedicated epoch rejects a same-handle destination ABA before source creation",
               context.mOwnerChecks == 1 && context.mReset && context.mReacquired && owner.hasTextureUploadDestinationGeneration() &&
                   !owner.hasTextureUploadSourceGeneration() && state.mCreatedTextureImages.size() == 2 &&
                   state.mCreatedTextureImages[0] == state.mCreatedTextureImages[1] && state.mCreateBufferCalls == 0);
        ensure("the replacement destination accepts a fresh exact source request",
               !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) && owner.reset());
    }

    {
        ...
        VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
        acquireTextureUploadDestinationChain(state, owner);
        const VkBuffer                       original_buffer         = state.mTextureUploadSourceBuffer;
        const VkDeviceMemory                 original_memory         = state.mTextureUploadSourceMemory;
        VulkanTextureUploadSourceDescription replacement_description = makeTextureUploadSourceDescription();
        for (std::size_t index = 0; index < replacement_description.mBytes.size(); ++index)
        {
            replacement_description.mBytes[index] ^= static_cast<std::uint8_t>(0xa5U + index);
        }
        TextureSourcePublicationContext        context{ &state, &owner, replacement_description, fakeHandle<VkBuffer>(0xf111),
                                                 fakeHandle<VkDeviceMemory>(0xf112) };
        const VulkanTextureUploadSourceRequest request{ owner.nativeWindowGeneration(),
                                                        makeTextureUploadSourceDescription(),
                                                        { &context, textureSourcePublicationOwnerIsCurrent },
                                                        { &context, textureSourcePublicationWindowIsCurrent } };
        ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                      VulkanTextureUploadSourceAcquireCode::TextureUploadSourceAlreadyOwned);
        ensure("a nested exact publication wins without being overwritten by the older source candidate",
               context.mAttempted && context.mPublished && context.mOwnerChecks == 2 && context.mWindowChecks == 1 &&
                   textureUploadSourceMatches(state, owner, context.mDescription) &&
                   owner.textureUploadSourceBuffer() == context.mReplacementBuffer &&
                   owner.textureUploadSourceMemory() == context.mReplacementMemory && state.mCreatedBuffers.size() == 2 &&
                   state.mAllocatedMemories.size() == 3);
        ensure("only the losing outer ownership occurrence rolls back",
               state.mDestroyedBuffers == std::vector<VkBuffer>{ original_buffer } &&
                   state.mFreedMemories == std::vector<VkDeviceMemory>{ original_memory } && state.mQueueSubmitCalls == 0 && owner.reset());
        ensure("the winning nested occurrence retires once at aggregate teardown",
               state.mDestroyedBuffers.size() == 2 && state.mDestroyedBuffers.back() == context.mReplacementBuffer &&
                   state.mFreedMemories.size() == 3 &&
                   std::find(state.mFreedMemories.begin(), state.mFreedMemories.end(), context.mReplacementMemory) !=
                       state.mFreedMemories.end());
    }
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<133>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadDestinationChain(state, owner);
    TextureSourceNativeGuardContext        context{ &owner };
    const VulkanTextureUploadSourceRequest request{ owner.nativeWindowGeneration(),
                                                    makeTextureUploadSourceDescription(),
                                                    { &context, textureSourceNativeGuardOwnerIsCurrent },
                                                    { &context, textureSourceNativeGuardWindowIsCurrent } };
    ensureTextureUploadSourceCode(owner.acquireTextureUploadSourceGeneration(request),
                                  VulkanTextureUploadSourceAcquireCode::StaleInstanceOwner);
    ensure("a live native source candidate blocks move, rebuild, and direct or transitive reset",
           context.mOwnerChecks == 2 && context.mWindowChecks == 1 && context.mMoveDestination &&
               context.mMoveDestination->instance() == VK_NULL_HANDLE && context.mRebuildError &&
               context.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
               context.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight && context.mRebuildOwnerChecks == 0 &&
               context.mRebuildWindowChecks == 0 && !context.mSourceResetSucceeded && !context.mDestinationResetSucceeded &&
               !context.mLogicalResetSucceeded && !context.mSurfaceResetSucceeded && !context.mFullResetSucceeded &&
               owner.hasTextureUploadDestinationGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               owner.hasLogicalDeviceGeneration() && state.mDestroyDeviceCalls == 0 && state.mDestroySurfaceCalls == 0);
    ensure("stale publication rolls the guarded native occurrence back after the callback returns",
           state.mDestroyBufferCalls == 1 && state.mFreeMemoryCalls == 1);
    ensure("the parent chain accepts a source after the native-acquisition guard leaves scope",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<134>()
{
    constexpr VkExtent2D     rebuilt_extent{ 1280, 720 };
    FakeState                state;
    ScopedFakeState          scope(state);
    ...
    acquireTextureUploadSourceChain(state, owner);
    acquireCompleteSwapchainChildren(state, owner);
    const VulkanTextureUploadSourceDescription source_description = makeTextureUploadSourceDescription();
    const VkImage                              image              = owner.textureUploadDestinationImage();
    const VkDeviceMemory                       image_memory       = owner.textureUploadDestinationMemory();
    const VkImageView                          image_view         = owner.textureUploadDestinationImageView();
    const VkBuffer                             source_buffer      = owner.textureUploadSourceBuffer();
    const VkDeviceMemory                       source_memory      = owner.textureUploadSourceMemory();

    ensureSwapchainChainRebuildOutcome(owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, rebuilt_extent)),
                                       VulkanSwapchainChainRebuildOutcome::Ready);
    ensure("changed-extent rebuild preserves the exact image and immutable source occurrences",
           textureUploadDestinationMatches(state, owner, vulkanTextureUploadDestinationDescription()) &&
               textureUploadSourceMatches(state, owner, source_description) && owner.textureUploadDestinationImage() == image &&
               owner.textureUploadDestinationMemory() == image_memory && owner.textureUploadDestinationImageView() == image_view &&
               owner.textureUploadSourceBuffer() == source_buffer && owner.textureUploadSourceMemory() == source_memory &&
               state.mQueueSubmitCalls == 0);

    ensure("direct source reset preserves the complete swapchain and image destination",
           owner.resetTextureUploadSourceGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && owner.textureUploadDestinationImage() == image &&
               owner.hasSwapchainConfigurationGeneration() && owner.hasSwapchainGeneration() && owner.hasSwapchainImagesGeneration() &&
               owner.hasSwapchainPresentationTargetGeneration() && owner.hasSwapchainPresentationPipelineGeneration() &&
               owner.hasSwapchainReadbackGeneration() && owner.hasSwapchainFrameSlotGeneration());
    ensure("the preserved image accepts a second same-bit source ownership occurrence",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner, source_description)) &&
               owner.textureUploadSourceBuffer() == source_buffer && owner.textureUploadSourceMemory() == source_memory);
    ...
    const VulkanSwapchainChainRebuildResult failed_result =
        owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1440, 810 }));
    const VulkanSwapchainChainRebuildError& failed = requireSwapchainChainRebuildError(failed_result);
    ensure("the forced changed-extent rebuild failure remains at the replacement swapchain",
           failed.mCode == VulkanSwapchainChainRebuildCode::ChildFailure && failed.mPhase == VulkanSwapchainChainRebuildPhase::Swapchain &&
               std::holds_alternative<VulkanSwapchainAcquireError>(failed.mChildError));
    ensure("failed rebuild rollback preserves the exact immutable texture source occurrence",
           textureUploadSourceMatches(state, owner, source_description) && owner.textureUploadSourceBuffer() == source_buffer &&
               owner.textureUploadSourceMemory() == source_memory &&
               std::find(state.mDestroyedBuffers.begin(), state.mDestroyedBuffers.end(), source_buffer) == state.mDestroyedBuffers.end() &&
               std::find(state.mFreedMemories.begin(), state.mFreedMemories.end(), source_memory) == state.mFreedMemories.end() &&
               state.mQueueSubmitCalls == 0);
    state.mSwapchainCreateResult = VK_SUCCESS;
    ...
    VulkanInstanceGeneration         moved(std::move(owner));
    const LLRenderContract::Extent2D moved_from_extent = owner.textureUploadSourceResidentExtent();
    ensure("aggregate move transfers the texture source and empties every moved-from source handle",
           owner.instance() == VK_NULL_HANDLE && !owner.hasTextureUploadSourceGeneration() && !owner.textureUploadSourceResourceHandle() &&
               owner.textureUploadSourceExpectedRevision() == 0 && moved_from_extent.mWidth == 0 && moved_from_extent.mHeight == 0 &&
               owner.textureUploadSourceContentIdentity() == 0 && owner.textureUploadSourceBuffer() == VK_NULL_HANDLE &&
               owner.textureUploadSourceMemory() == VK_NULL_HANDLE && textureUploadSourceMatches(state, moved, source_description));

    state.mDeviceDestroyOwner  = &moved;
    state.mSurfaceDestroyOwner = &moved;
    state.mEvents.clear();
    ensure("surface reset cascades through source, image destination, and logical device", moved.resetSurfaceGeneration());
    const auto source_buffer_destroy = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureUploadSourceBuffer);
    const auto source_memory_free    = std::find(state.mEvents.begin(), state.mEvents.end(), Event::FreeTextureUploadSourceMemory);
    const auto image_view_destroy    = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    const auto image_destroy         = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImage);
    const auto image_memory_free     = std::find(state.mEvents.begin(), state.mEvents.end(), Event::FreeTextureMemory);
    const auto device_destroy        = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyDevice);
    const auto surface_destroy       = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroySurface);
    ensure("source buffer and memory retire before image view, image, memory, device, and surface",
           source_buffer_destroy != state.mEvents.end() && source_memory_free != state.mEvents.end() &&
               image_view_destroy != state.mEvents.end() && image_destroy != state.mEvents.end() &&
               image_memory_free != state.mEvents.end() && device_destroy != state.mEvents.end() &&
               surface_destroy != state.mEvents.end() && source_buffer_destroy < source_memory_free &&
               source_memory_free < image_view_destroy && image_view_destroy < image_destroy && image_destroy < image_memory_free &&
               image_memory_free < device_destroy && device_destroy < surface_destroy && state.mDeviceDestroyObservationMade &&
               !state.mObservedTextureUploadSourceAtDeviceDestroy && !state.mObservedTextureUploadDestinationAtDeviceDestroy &&
               state.mSurfaceDestroyObservationMade && !state.mObservedTextureUploadSourceAtSurfaceDestroy &&
               !state.mObservedTextureUploadDestinationAtSurfaceDestroy && !moved.hasTextureUploadSourceGeneration() &&
               !moved.hasTextureUploadDestinationGeneration() && !moved.hasLogicalDeviceGeneration() &&
               !moved.hasPresentationDeviceGeneration() && !moved.hasSurfaceGeneration());
    ensure("the remaining instance generation resets after the source dependency cascade", moved.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<135>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadSourceChain(state, owner);
    acquireCompleteSwapchainChildren(state, owner);
    state.mTextureUploadSourceTeardownReentryOwner = &owner;

    ensure("direct source reset completes after guarded buffer and memory callbacks", owner.resetTextureUploadSourceGeneration());
    ensure("the source slot detached before callbacks while every parent and independent swapchain child survived",
           !owner.hasTextureUploadSourceGeneration() && owner.hasTextureUploadDestinationGeneration() &&
               owner.hasLogicalDeviceGeneration() && owner.hasPresentationDeviceGeneration() && owner.hasSurfaceGeneration() &&
               owner.hasSwapchainConfigurationGeneration() && owner.hasSwapchainGeneration() && owner.hasSwapchainImagesGeneration() &&
               owner.hasSwapchainPresentationTargetGeneration() && owner.hasSwapchainPresentationPipelineGeneration() &&
               owner.hasSwapchainReadbackGeneration() && owner.hasSwapchainFrameSlotGeneration() && state.mDestroyBufferCalls == 1 &&
               state.mFreeMemoryCalls == 1 && state.mTextureUploadSourceTeardownObservations.size() == 2);

    for (const TextureUploadSourceTeardownObservation& observation : state.mTextureUploadSourceTeardownObservations)
    {
        ensure("each native teardown callback sees a detached source and live destination parents",
               observation.mSourceDetached && observation.mDestinationLive && observation.mLogicalDeviceLive && observation.mSurfaceLive);
        ensure("nested source acquisition reports the dedicated teardown error throughout retirement",
               observation.mAcquireError &&
                   observation.mAcquireError->mCode == VulkanTextureUploadSourceAcquireCode::NativeTeardownInProgress &&
                   !observation.mAcquireError->mResolutionError);
        ensure("sample-binding acquisition reports the dedicated teardown error throughout source retirement",
               observation.mBindingAcquireError &&
                   observation.mBindingAcquireError->mCode == VulkanTextureUploadSampleBindingAcquireCode::NativeTeardownInProgress &&
                   !observation.mBindingAcquireError->mResolutionError);
        ensure("direct source retirement preserves configuration but rejects configuration reacquisition without native work",
               !observation.mSwapchainConfigurationDetached && observation.mConfigurationAcquireError &&
                   observation.mConfigurationAcquireError->mCode == VulkanSwapchainConfigurationAcquireCode::NativeTeardownInProgress &&
                   !observation.mConfigurationAcquireError->mResolutionError && observation.mConfigurationNativeWorkUnchanged);
        ensure("move, rebuild, and direct or transitive resets remain blocked throughout retirement",
               observation.mMoveProducedEmpty && observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight && !observation.mSourceResetSucceeded &&
                   !observation.mDestinationResetSucceeded && !observation.mLogicalResetSucceeded && !observation.mSurfaceResetSucceeded &&
                   !observation.mFullResetSucceeded);
    }

    state.mTextureUploadSourceTeardownReentryOwner = nullptr;
    ensure("the preserved dependency chain accepts a new source once native teardown leaves scope",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) &&
               owner.hasTextureUploadSourceGeneration() && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<136>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ...
    acquireCompleteSwapchainChildren(state, owner);
    state.mTextureUploadSourceTeardownReentryOwner = &owner;
    state.mDeviceDestroyOwner                      = &owner;

    ensure("logical-device reset completes after retiring the full swapchain chain and texture source",
           owner.resetLogicalDeviceGeneration());
    ensure_equals("both texture-source native retirement callbacks were observed", state.mTextureUploadSourceTeardownObservations.size(),
                  std::size_t{ 2 });
    for (const TextureUploadSourceTeardownObservation& observation : state.mTextureUploadSourceTeardownObservations)
    {
        ensure("parent teardown detaches swapchain configuration and source before each source callback while parents stay live",
               observation.mSourceDetached && observation.mSwapchainConfigurationDetached && observation.mDestinationLive &&
                   observation.mLogicalDeviceLive && observation.mSurfaceLive);
        ensure("both source and configuration reacquisition report their dedicated teardown errors without configuration native work",
               observation.mAcquireError &&
                   observation.mAcquireError->mCode == VulkanTextureUploadSourceAcquireCode::NativeTeardownInProgress &&
                   !observation.mAcquireError->mResolutionError && observation.mConfigurationAcquireError &&
                   observation.mConfigurationAcquireError->mCode == VulkanSwapchainConfigurationAcquireCode::NativeTeardownInProgress &&
                   !observation.mConfigurationAcquireError->mResolutionError && observation.mConfigurationNativeWorkUnchanged);
        ensure("sample-binding acquisition remains blocked throughout transitive source retirement",
               observation.mBindingAcquireError &&
                   observation.mBindingAcquireError->mCode == VulkanTextureUploadSampleBindingAcquireCode::NativeTeardownInProgress &&
                   !observation.mBindingAcquireError->mResolutionError);
        ensure("move, rebuild, and direct or transitive resets stay blocked for the complete parent teardown",
               observation.mMoveProducedEmpty && observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight && !observation.mSourceResetSucceeded &&
                   !observation.mDestinationResetSucceeded && !observation.mLogicalResetSucceeded && !observation.mSurfaceResetSucceeded &&
    ...
    ensure("outer reset leaves no device-scoped or swapchain child before destroying the logical device",
           !owner.hasLogicalDeviceGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               !owner.hasTextureUploadDestinationGeneration() && !owner.hasSwapchainConfigurationGeneration() &&
               !owner.hasSwapchainGeneration() && !owner.hasSwapchainImagesGeneration() &&
               !owner.hasSwapchainPresentationTargetGeneration() && !owner.hasSwapchainPresentationPipelineGeneration() &&
               !owner.hasSwapchainReadbackGeneration() && !owner.hasSwapchainFrameSlotGeneration() && state.mDestroyDeviceCalls == 1 &&
               state.mDeviceDestroyObservationMade && !state.mObservedConfigurationAtDeviceDestroy &&
               !state.mObservedSwapchainAtDeviceDestroy && !state.mObservedSwapchainImagesAtDeviceDestroy &&
               !state.mObservedFrameSlotAtDeviceDestroy && !state.mObservedUploadSourceAtDeviceDestroy &&
               !state.mObservedTextureUploadDestinationAtDeviceDestroy && !state.mObservedTextureUploadSourceAtDeviceDestroy &&
               !state.mObservedUploadDestinationAtDeviceDestroy && !state.mObservedUploadTransferAtDeviceDestroy);

    state.mTextureUploadSourceTeardownReentryOwner = nullptr;
    state.mDeviceDestroyOwner                      = nullptr;
    ensure("remaining instance parents reset after guarded logical-device teardown", owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<137>()
{
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().acquireTextureUploadTransferGeneration(
        std::declval<const VulkanTextureUploadTransferRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().executeTextureUploadTransfer(
        std::declval<const VulkanTextureUploadTransferOperationRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().retryTextureUploadTransferCompletion(
        std::declval<const VulkanTextureUploadTransferOperationRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().resetTextureUploadTransferGeneration()));

    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ensure("an inert aggregate exposes no texture-transfer or destination residency state",
           !owner.hasTextureUploadTransferGeneration() && !owner.textureUploadTransferResourceHandle() &&
               owner.textureUploadTransferExpectedRevision() == 0 && owner.textureUploadTransferContentIdentity() == 0 &&
               owner.textureUploadTransferSourceBuffer() == VK_NULL_HANDLE &&
               owner.textureUploadTransferDestinationImage() == VK_NULL_HANDLE && owner.textureUploadTransferQueue() == VK_NULL_HANDLE &&
               owner.textureUploadTransferQueueFamilyIndex() == VK_QUEUE_FAMILY_IGNORED &&
               owner.textureUploadTransferQueueIndex() == std::numeric_limits<std::uint32_t>::max() &&
               owner.textureUploadTransferCommandPool() == VK_NULL_HANDLE && owner.textureUploadTransferCommandBuffer() == VK_NULL_HANDLE &&
               owner.textureUploadTransferFence() == VK_NULL_HANDLE && owner.textureUploadTransferSubmissionCount() == 0 &&
               owner.textureUploadTransferCompletionWaitCount() == 0 && !owner.textureUploadTransferDisposition() &&
               !owner.textureUploadDestinationIsResident() && owner.textureUploadDestinationResidentRevision() == 0 &&
               owner.textureUploadDestinationResidentContentIdentity() == 0 &&
               owner.textureUploadDestinationCurrentState() == LLRenderContract::ImageState::Undefined);

    const VulkanTextureUploadTransferAcquireResult missing =
        owner.acquireTextureUploadTransferGeneration(makeTextureUploadTransferRequest(state, owner));
    ensure("texture transfer acquisition requires the texture source and destination chain without native work",
           missing && missing->mCode == VulkanTextureUploadTransferAcquireCode::SurfaceNotLive &&
               !owner.hasTextureUploadTransferGeneration());
    const VulkanTextureUploadTransferParentOperationResult operation =
        owner.executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, owner));
    const auto* operation_error = std::get_if<VulkanTextureUploadTransferParentOperationError>(&operation);
    ensure("texture transfer execution requires the same live parent chain without native work",
           operation_error && operation_error->mCode == VulkanTextureUploadTransferParentOperationCode::SurfaceNotLive &&
               !operation_error->mOperationError && owner.resetTextureUploadTransferGeneration() && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<138>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadTransferChain(state, owner);
    ensure("texture transfer publication retains the exact source and destination",
           owner.hasTextureUploadTransferGeneration() &&
               owner.textureUploadTransferResourceHandle() == owner.textureUploadSourceResourceHandle() &&
               owner.textureUploadTransferExpectedRevision() == owner.textureUploadSourceExpectedRevision() &&
               owner.textureUploadTransferContentIdentity() == owner.textureUploadSourceContentIdentity() &&
               owner.textureUploadTransferSourceBuffer() == owner.textureUploadSourceBuffer() &&
               owner.textureUploadTransferDestinationImage() == owner.textureUploadDestinationImage() &&
               owner.textureUploadTransferDisposition() == VulkanTextureUploadTransferDisposition::Ready &&
               !owner.textureUploadDestinationIsResident());

    state.mTextureUploadTransferOperationReentryOwner = &owner;
    const auto  result      = owner.executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, owner));
    const auto* disposition = std::get_if<VulkanTextureUploadTransferDisposition>(&result);
    ensure("one submission completes and publishes shader-readable destination residency",
           disposition && *disposition == VulkanTextureUploadTransferDisposition::Complete &&
               owner.textureUploadTransferDisposition() == VulkanTextureUploadTransferDisposition::Complete &&
               owner.textureUploadTransferSubmissionCount() == 1 && owner.textureUploadTransferCompletionWaitCount() == 1 &&
               state.mQueueSubmitCalls == 1 && state.mWaitForFencesCalls == 1 && owner.textureUploadDestinationIsResident() &&
               owner.textureUploadDestinationResidentRevision() == owner.textureUploadSourceExpectedRevision() &&
               owner.textureUploadDestinationResidentContentIdentity() == owner.textureUploadSourceContentIdentity() &&
               owner.textureUploadDestinationCurrentState() == LLRenderContract::ImageState::ShaderRead);
    ensure("recursive aggregate execution is rejected before recording or submitting twice",
           state.mTextureUploadTransferOperationReentryInvoked && state.mTextureUploadTransferOperationReentryError &&
               state.mTextureUploadTransferOperationReentryError->mCode ==
                   VulkanTextureUploadTransferParentOperationCode::NativeOperationInProgress &&
               !state.mTextureUploadTransferOperationReentryError->mOperationError && state.mQueueSubmitCalls == 1);

    state.mTextureUploadTransferOperationReentryOwner = nullptr;
    state.mTextureUploadTransferTeardownOwner         = &owner;
    ensure("direct completed-transfer reset preserves source, destination, and published residency",
           owner.resetTextureUploadTransferGeneration() && !owner.hasTextureUploadTransferGeneration() &&
               owner.hasTextureUploadSourceGeneration() && owner.hasTextureUploadDestinationGeneration() &&
               owner.textureUploadDestinationIsResident() && state.mTextureUploadTransferTeardownObservations.size() == 2);
    for (const auto& observation : state.mTextureUploadTransferTeardownObservations)
    {
        ensure("both transfer callbacks reject publication and operations before authentication or native work",
               observation.mTransferDetached && observation.mMoveProducedEmpty && observation.mNoAuthOrNativeWork &&
                   observation.mAcquireError &&
                   observation.mAcquireError->mCode == VulkanTextureUploadTransferAcquireCode::NativeTeardownInProgress &&
                   !observation.mAcquireError->mResolutionError && observation.mBindingAcquireError &&
                   observation.mBindingAcquireError->mCode == VulkanTextureUploadSampleBindingAcquireCode::NativeTeardownInProgress &&
                   !observation.mBindingAcquireError->mResolutionError && observation.mConfigurationAcquireError &&
                   observation.mConfigurationAcquireError->mCode == VulkanSwapchainConfigurationAcquireCode::NativeTeardownInProgress &&
                   !observation.mConfigurationAcquireError->mResolutionError && observation.mOperationError &&
                   observation.mOperationError->mCode == VulkanTextureUploadTransferParentOperationCode::NativeOperationInProgress &&
                   !observation.mOperationError->mOperationError && observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight &&
                   !observation.mTransferResetSucceeded && !observation.mSourceResetSucceeded && !observation.mDestinationResetSucceeded &&
                   !observation.mLogicalResetSucceeded && !observation.mFullResetSucceeded);
    }
    state.mTextureUploadTransferTeardownOwner = nullptr;
    ensure("remaining texture upload owners reset after direct transfer retirement", owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<139>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ...
    const auto  execute = owner.executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, owner, 37));
    const auto* timeout = std::get_if<VulkanTextureUploadTransferParentOperationError>(&execute);
    ensure("a bounded timeout retains pending ownership without publishing residency",
           timeout && timeout->mCode == VulkanTextureUploadTransferParentOperationCode::OperationFailure && timeout->mOperationError &&
               timeout->mOperationError->mCommand == VulkanTextureUploadTransferCommand::WaitForFences &&
               timeout->mOperationError->mResult == VK_TIMEOUT &&
               timeout->mOperationError->mDisposition == VulkanTextureUploadTransferDisposition::Pending &&
               owner.textureUploadTransferDisposition() == VulkanTextureUploadTransferDisposition::Pending &&
               owner.textureUploadTransferSubmissionCount() == 1 && owner.textureUploadTransferCompletionWaitCount() == 1 &&
               state.mWaitTimeout == 37 && state.mQueueSubmitCalls == 1 && !owner.textureUploadDestinationIsResident());
    ensure("pending transfer blocks every direct and transitive reset before any teardown",
           !owner.resetTextureUploadTransferGeneration() && !owner.resetTextureUploadSourceGeneration() &&
               !owner.resetTextureUploadDestinationGeneration() && !owner.resetLogicalDeviceGeneration() && !owner.reset() &&
               owner.hasTextureUploadTransferGeneration() && owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && state.mDestroyFenceCalls == 0 && state.mDestroyCommandPoolCalls == 0);

    const VkCommandPool transfer_pool = owner.textureUploadTransferCommandPool();
    ensureSwapchainChainRebuildOutcome(owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1280, 720 })),
                                       VulkanSwapchainChainRebuildOutcome::Ready);
    ensure("changed-extent rebuild preserves the independent pending texture-transfer chain",
           owner.textureUploadTransferDisposition() == VulkanTextureUploadTransferDisposition::Pending &&
               owner.textureUploadTransferCommandPool() == transfer_pool && owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && !owner.textureUploadDestinationIsResident());

    const auto  retry    = owner.retryTextureUploadTransferCompletion(makeTextureUploadTransferOperationRequest(state, owner, 73));
    const auto* complete = std::get_if<VulkanTextureUploadTransferDisposition>(&retry);
    ensure("completion retry waits only and never records or submits again",
           complete && *complete == VulkanTextureUploadTransferDisposition::Complete && state.mQueueSubmitCalls == 1 &&
               state.mWaitForFencesCalls == 2 && state.mWaitTimeout == 73 && owner.textureUploadTransferSubmissionCount() == 1 &&
               owner.textureUploadTransferCompletionWaitCount() == 2 && owner.textureUploadDestinationIsResident() && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<140>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadTransferChain(state, owner);
    acquireCompleteSwapchainChildren(state, owner);
    const VkCommandPool pool     = owner.textureUploadTransferCommandPool();
    const VkFence       fence    = owner.textureUploadTransferFence();
    state.mSwapchainCreateResult = VK_ERROR_OUT_OF_HOST_MEMORY;
    const auto  failed_result    = owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1280, 720 }));
    const auto& failed           = requireSwapchainChainRebuildError(failed_result);
    ensure("failed changed-extent rebuild preserves the ready texture-transfer chain exactly",
           failed.mCode == VulkanSwapchainChainRebuildCode::ChildFailure && failed.mPhase == VulkanSwapchainChainRebuildPhase::Swapchain &&
               owner.hasTextureUploadTransferGeneration() && owner.textureUploadTransferCommandPool() == pool &&
               owner.textureUploadTransferFence() == fence &&
               owner.textureUploadTransferDisposition() == VulkanTextureUploadTransferDisposition::Ready &&
               owner.hasTextureUploadSourceGeneration() && owner.hasTextureUploadDestinationGeneration() && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<141>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadTransferChain(state, owner);
    state.mTextureUploadDestinationTeardownOwner = &owner;

    ensure("destination reset retires transfer and source before the guarded image owner",
           owner.resetTextureUploadDestinationGeneration() && !owner.hasTextureUploadTransferGeneration() &&
               !owner.hasTextureUploadSourceGeneration() && !owner.hasTextureUploadDestinationGeneration() &&
               state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1 &&
               state.mTextureUploadDestinationTeardownObservations.size() == 3);
    for (const auto& observation : state.mTextureUploadDestinationTeardownObservations)
    {
        ensure("each destination callback observes detached aggregate accessors and teardown-specific publication rejection",
               observation.mDestinationDetached && observation.mMoveProducedEmpty && !observation.mDestinationResetSucceeded &&
                   !observation.mLogicalResetSucceeded && !observation.mFullResetSucceeded && observation.mDestinationAcquireError &&
                   observation.mDestinationAcquireError->mCode == VulkanTextureUploadDestinationAcquireCode::NativeTeardownInProgress &&
                   !observation.mDestinationAcquireError->mResolutionError && observation.mBindingAcquireError &&
                   observation.mBindingAcquireError->mCode == VulkanTextureUploadSampleBindingAcquireCode::NativeTeardownInProgress &&
                   !observation.mBindingAcquireError->mResolutionError && observation.mConfigurationAcquireError &&
                   observation.mConfigurationAcquireError->mCode == VulkanSwapchainConfigurationAcquireCode::NativeTeardownInProgress &&
                   !observation.mConfigurationAcquireError->mResolutionError && observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight);
    }
    const auto view   = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    const auto image  = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImage);
    const auto memory = std::find(state.mEvents.begin(), state.mEvents.end(), Event::FreeTextureMemory);
    ensure("destination native teardown occurs exactly once in view-image-memory order",
           view != state.mEvents.end() && image != state.mEvents.end() && memory != state.mEvents.end() && view < image && image < memory);

    state.mTextureUploadDestinationTeardownOwner = nullptr;
    ensure("logical parents remain resettable after guarded destination retirement", owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<142>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadDestinationChain(state, owner);
    ensure("the transitive fixture acquires a legacy upload source on the shared logical device",
           !owner.acquireUploadSourceGeneration(makeUploadSourceRequest(state, owner)));
    state.mMemoryProperties.memoryTypes[0].propertyFlags |= VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
    ensure("the transitive fixture acquires a legacy upload destination and ready transfer",
           !owner.acquireUploadDestinationGeneration(makeUploadDestinationRequest(state, owner)) &&
               !owner.acquireUploadTransferGeneration(makeUploadTransferRequest(state, owner)) &&
               owner.uploadTransferDisposition() == VulkanUploadTransferDisposition::Ready);
    acquireCompleteSwapchainChildren(state, owner);
    state.mTextureUploadDestinationTeardownOwner = &owner;
    state.mDeviceDestroyOwner                    = &owner;

    ensure("logical-device reset transitively retires swapchain children before the guarded texture destination",
           owner.resetLogicalDeviceGeneration() && !owner.hasLogicalDeviceGeneration() && !owner.hasTextureUploadDestinationGeneration() &&
               !owner.hasSwapchainConfigurationGeneration() && !owner.hasSwapchainGeneration() && !owner.hasSwapchainImagesGeneration() &&
               !owner.hasSwapchainFrameSlotGeneration() && state.mTextureUploadDestinationTeardownObservations.size() == 3);
    for (const auto& observation : state.mTextureUploadDestinationTeardownObservations)
    {
        ensure("every transitive destination callback rejects destination and swapchain-root republication with typed teardown errors",
               observation.mDestinationDetached && observation.mMoveProducedEmpty && !observation.mDestinationResetSucceeded &&
                   !observation.mLogicalResetSucceeded && !observation.mFullResetSucceeded && observation.mDestinationAcquireError &&
                   observation.mDestinationAcquireError->mCode == VulkanTextureUploadDestinationAcquireCode::NativeTeardownInProgress &&
                   !observation.mDestinationAcquireError->mResolutionError && observation.mBindingAcquireError &&
                   ...
                   !observation.mConfigurationAcquireError->mResolutionError && observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight && observation.mLegacyOperationError &&
                   observation.mLegacyOperationError->mCode == VulkanUploadTransferParentOperationCode::NativeOperationInProgress &&
                   !observation.mLegacyOperationError->mOperationError && observation.mLegacyOperationUnchanged);
    }
    const auto view   = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    const auto image  = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImage);
    const auto memory = std::find(state.mEvents.begin(), state.mEvents.end(), Event::FreeTextureMemory);
    const auto device = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyDevice);
    ensure("destination retires exactly once in view-image-memory order before a child-free device destruction",
           state.mDestroyTextureImageViewCalls == 1 && state.mDestroyTextureImageCalls == 1 && state.mFreeTextureMemoryCalls == 1 &&
               view != state.mEvents.end() && image != state.mEvents.end() && memory != state.mEvents.end() &&
               device != state.mEvents.end() && view < image && image < memory && memory < device && state.mDestroyDeviceCalls == 1 &&
               state.mDeviceDestroyObservationMade && !state.mObservedTextureUploadDestinationAtDeviceDestroy &&
               !state.mObservedConfigurationAtDeviceDestroy && !state.mObservedSwapchainAtDeviceDestroy &&
               !state.mObservedSwapchainImagesAtDeviceDestroy && !state.mObservedFrameSlotAtDeviceDestroy);

    state.mTextureUploadDestinationTeardownOwner = nullptr;
    state.mDeviceDestroyOwner                    = nullptr;
    ensure("remaining instance parents reset after transitive destination teardown", owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<143>()
{
    static_assert(noexcept(std::declval<const VulkanInstanceGeneration&>().hasTextureUploadSampleBindingGeneration()));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().acquireTextureUploadSampleBindingGeneration(
        std::declval<const VulkanTextureUploadSampleBindingRequest&>())));
    static_assert(noexcept(std::declval<VulkanInstanceGeneration&>().resetTextureUploadSampleBindingGeneration()));

    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ensure("an absent sampled binding exposes only inert metadata",
           !owner.hasTextureUploadSampleBindingGeneration() && !owner.textureUploadSampleBindingSamplerResourceHandle() &&
               !owner.textureUploadSampleBindingDestinationResourceHandle() && owner.textureUploadSampleBindingExpectedRevision() == 0 &&
               owner.textureUploadSampleBindingResidentRevision() == 0 && owner.textureUploadSampleBindingResidentContentIdentity() == 0 &&
               owner.textureUploadSampleBindingDestinationImageView() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingDestinationImageLayout() == VK_IMAGE_LAYOUT_MAX_ENUM &&
               owner.textureUploadSampleBindingDescriptorSetIndex() == std::numeric_limits<std::uint32_t>::max() &&
               owner.textureUploadSampleBindingBinding() == std::numeric_limits<std::uint32_t>::max() &&
               owner.textureUploadSampleBindingSampler() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingDescriptorSetLayout() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingPipelineLayout() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingDescriptorPool() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingDescriptorSet() == VK_NULL_HANDLE);
    ensureTextureUploadSampleBindingCode(
        owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)),
        VulkanTextureUploadSampleBindingAcquireCode::SurfaceNotLive);

    acquireTextureUploadDestinationChain(state, owner);
    const VulkanTextureUploadSampleBindingAcquireResult nonresident =
        owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner));
    const auto& nonresident_error = requireTextureUploadSampleBindingError(nonresident);
    ensure("an unpublished destination returns the exact standalone residency failure without creating binding objects",
           nonresident_error.mCode == VulkanTextureUploadSampleBindingAcquireCode::ResolutionFailure &&
               nonresident_error.mResolutionError &&
               nonresident_error.mResolutionError->mCode == VulkanTextureUploadSampleBindingResolutionCode::DestinationNotResident &&
               state.mCreateTextureSampleSamplerCalls == 0);

    ensure("the resident fixture source and transfer publish",
           !owner.acquireTextureUploadSourceGeneration(makeTextureUploadSourceRequest(state, owner)) &&
               !owner.acquireTextureUploadTransferGeneration(makeTextureUploadTransferRequest(state, owner)));
    const auto  transfer_result      = owner.executeTextureUploadTransfer(makeTextureUploadTransferOperationRequest(state, owner));
    const auto* transfer_disposition = std::get_if<VulkanTextureUploadTransferDisposition>(&transfer_result);
    ensure("the exact texture transfer completes before binding acquisition",
           transfer_disposition && *transfer_disposition == VulkanTextureUploadTransferDisposition::Complete &&
               owner.textureUploadDestinationIsResident());

    const VulkanTextureUploadSampleBindingDescription description = vulkanTextureUploadSampleBindingDescription();
    ensure("one canonical sampled binding publishes",
           !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner, description)) &&
               textureUploadSampleBindingMatches(state, owner));
    ensure("the sampled binding creates and writes one exact fixed descriptor interface",
           state.mCreateTextureSampleSamplerCalls == 1 && state.mTextureSampleSamplerCreateInfo.magFilter == VK_FILTER_LINEAR &&
               state.mTextureSampleSamplerCreateInfo.minFilter == VK_FILTER_LINEAR &&
               state.mTextureSampleSamplerCreateInfo.mipmapMode == VK_SAMPLER_MIPMAP_MODE_LINEAR &&
               state.mTextureSampleSamplerCreateInfo.addressModeU == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE &&
               state.mTextureSampleSamplerCreateInfo.addressModeV == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE &&
               state.mTextureSampleSamplerCreateInfo.addressModeW == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE &&
               state.mTextureSampleSamplerCreateInfo.minLod == 0.f && state.mTextureSampleSamplerCreateInfo.maxLod == 2.f &&
               state.mTextureSampleSamplerCreateInfo.anisotropyEnable == VK_FALSE &&
               state.mTextureSampleSamplerCreateInfo.compareEnable == VK_FALSE && state.mCreateTextureSampleDescriptorSetLayoutCalls == 1 &&
               state.mTextureSampleLayoutBinding.binding == 0 &&
               state.mTextureSampleLayoutBinding.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER &&
               state.mTextureSampleLayoutBinding.descriptorCount == 1 &&
               state.mTextureSampleLayoutBinding.stageFlags == VK_SHADER_STAGE_FRAGMENT_BIT &&
               state.mCreateTextureSamplePipelineLayoutCalls == 1 && state.mTextureSamplePipelineLayoutCreateInfo.setLayoutCount == 1 &&
               state.mTextureSamplePipelineSetLayout == state.mTextureSampleDescriptorSetLayout &&
               state.mCreateTextureSampleDescriptorPoolCalls == 1 && state.mTextureSampleDescriptorPoolCreateInfo.maxSets == 1 &&
               state.mTextureSampleDescriptorPoolSize.type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER &&
               state.mTextureSampleDescriptorPoolSize.descriptorCount == 1 && state.mAllocateTextureSampleDescriptorSetCalls == 1 &&
               state.mTextureSampleDescriptorSetAllocateInfo.descriptorPool == state.mTextureSampleDescriptorPool &&
               state.mTextureSampleAllocatedSetLayout == state.mTextureSampleDescriptorSetLayout &&
               state.mUpdateTextureSampleDescriptorSetCalls == 1 &&
               state.mTextureSampleDescriptorWrite.dstSet == state.mTextureSampleDescriptorSet &&
               state.mTextureSampleDescriptorWrite.dstBinding == 0 &&
               state.mTextureSampleDescriptorWrite.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER &&
               state.mTextureSampleDescriptorImageInfo.sampler == state.mTextureSampleSampler &&
               state.mTextureSampleDescriptorImageInfo.imageView == state.mTextureUploadDestinationView &&
               state.mTextureSampleDescriptorImageInfo.imageLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    const std::size_t sampler_creates = state.mCreateTextureSampleSamplerCalls;
    const std::size_t lookups         = state.mDeviceProcAddrCalls;
    ensureTextureUploadSampleBindingCode(
        owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)),
        VulkanTextureUploadSampleBindingAcquireCode::TextureUploadSampleBindingAlreadyOwned);
    ensure("duplicate binding acquisition stops before callbacks, dispatch, or native work",
           state.mCreateTextureSampleSamplerCalls == sampler_creates && state.mDeviceProcAddrCalls == lookups);

    state.mEvents.clear();
    ensure("direct binding reset preserves its destination, source, transfer, and logical device",
           owner.resetTextureUploadSampleBindingGeneration() && !owner.hasTextureUploadSampleBindingGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadTransferGeneration() && owner.hasLogicalDeviceGeneration());
    const auto pool_destroy     = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSampleDescriptorPool);
    const auto pipeline_destroy = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSamplePipelineLayout);
    const auto layout_destroy   = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSampleDescriptorSetLayout);
    const auto sampler_destroy  = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSampleSampler);
    ensure("binding reset destroys the pool, pipeline layout, set layout, and sampler in dependency order",
           pool_destroy != state.mEvents.end() && pipeline_destroy != state.mEvents.end() && layout_destroy != state.mEvents.end() &&
               sampler_destroy != state.mEvents.end() && pool_destroy < pipeline_destroy && pipeline_destroy < layout_destroy &&
               layout_destroy < sampler_destroy);
    ensure("an absent binding reset is idempotently successful without another destroy",
           owner.resetTextureUploadSampleBindingGeneration() && state.mDestroyTextureSampleSamplerCalls == 1);
    ensure("the reset slot reacquires a second same-bit ownership occurrence",
           !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)) &&
               textureUploadSampleBindingMatches(state, owner) && state.mCreateTextureSampleSamplerCalls == 2);

    VulkanInstanceGeneration moved(std::move(owner));
    ensure("aggregate move transfers the sampled binding and empties all moved-from accessors",
           owner.instance() == VK_NULL_HANDLE && !owner.hasTextureUploadSampleBindingGeneration() &&
               owner.textureUploadSampleBindingSampler() == VK_NULL_HANDLE &&
               owner.textureUploadSampleBindingDescriptorSet() == VK_NULL_HANDLE && textureUploadSampleBindingMatches(state, moved) &&
               moved.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<144>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        ...
        VulkanTextureUploadSampleBindingRequest request = makeTextureUploadSampleBindingRequest(state, owner);
        request.mInstanceOwnerCheck.mIsCurrent          = nullptr;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::InvalidInstanceOwnerCheck);
        request                                   = makeTextureUploadSampleBindingRequest(state, owner);
        request.mWindowGenerationCheck.mIsCurrent = nullptr;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::InvalidWindowGenerationCheck);
        request                         = makeTextureUploadSampleBindingRequest(state, owner);
        request.mNativeWindowGeneration = 0;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::InvalidNativeWindowGeneration);
        request                         = makeTextureUploadSampleBindingRequest(state, owner);
        request.mNativeWindowGeneration = 43;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::NativeWindowGenerationMismatch);
        request                     = makeTextureUploadSampleBindingRequest(state, owner);
        state.mInstanceOwnerCurrent = false;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::StaleInstanceOwner);
        state.mInstanceOwnerCurrent = true;
        state.mSurfaceWindowCurrent = false;
        ensureTextureUploadSampleBindingCode(owner.acquireTextureUploadSampleBindingGeneration(request),
                                             VulkanTextureUploadSampleBindingAcquireCode::StaleWindowGeneration);
        state.mSurfaceWindowCurrent = true;

        request = makeTextureUploadSampleBindingRequest(state, owner);
        ++request.mDestinationDescription.mExpectedRevision;
        const auto  invalid_destination       = owner.acquireTextureUploadSampleBindingGeneration(request);
        const auto& invalid_destination_error = requireTextureUploadSampleBindingError(invalid_destination);
        ensure("the explicit noncanonical destination description is rejected by the standalone resolver",
               invalid_destination_error.mCode == VulkanTextureUploadSampleBindingAcquireCode::ResolutionFailure &&
                   invalid_destination_error.mResolutionError &&
                   invalid_destination_error.mResolutionError->mCode == VulkanTextureUploadSampleBindingResolutionCode::InvalidDescription);
        request                           = makeTextureUploadSampleBindingRequest(state, owner);
        request.mDescription.mBinding     = 1;
        const auto  invalid_binding       = owner.acquireTextureUploadSampleBindingGeneration(request);
        const auto& invalid_binding_error = requireTextureUploadSampleBindingError(invalid_binding);
        ensure("the noncanonical sampled interface is rejected without native creation",
               invalid_binding_error.mCode == VulkanTextureUploadSampleBindingAcquireCode::ResolutionFailure &&
                   invalid_binding_error.mResolutionError &&
                   invalid_binding_error.mResolutionError->mCode == VulkanTextureUploadSampleBindingResolutionCode::InvalidDescription &&
                   state.mCreateTextureSampleSamplerCalls == 0);

        state.mMissing            = MissingCommand::CreateSampler;
        const auto  missing       = owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner));
        const auto& missing_error = requireTextureUploadSampleBindingError(missing);
        ensure("the aggregate preserves an exact missing-dispatch failure",
               missing_error.mCode == VulkanTextureUploadSampleBindingAcquireCode::ResolutionFailure && missing_error.mResolutionError &&
                   missing_error.mResolutionError->mCode == VulkanTextureUploadSampleBindingResolutionCode::MissingRequiredCommand &&
                   missing_error.mResolutionError->mCommand == VulkanTextureUploadSampleBindingCommand::CreateSampler &&
                   !owner.hasTextureUploadSampleBindingGeneration());
        state.mMissing = MissingCommand::None;
        ensure("the resident destination remains reusable after invalid and missing-command failures",
               !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)) && owner.reset());
    }

    {
        ...
        acquireResidentTextureUploadDestination(state, owner);
        state.mFailInstanceOwnerCheck = state.mInstanceOwnerChecks + 2;
        const auto stale = owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner));
        ensureTextureUploadSampleBindingCode(stale, VulkanTextureUploadSampleBindingAcquireCode::StaleInstanceOwner);
        ensure("post-resolution staleness rolls every unpublished native binding object back",
               !owner.hasTextureUploadSampleBindingGeneration() && state.mCreateTextureSampleSamplerCalls == 1 &&
                   state.mDestroyTextureSampleSamplerCalls == 1 && state.mDestroyTextureSampleDescriptorSetLayoutCalls == 1 &&
                   state.mDestroyTextureSamplePipelineLayoutCalls == 1 && state.mDestroyTextureSampleDescriptorPoolCalls == 1);
        state.mFailInstanceOwnerCheck = 0;
        ensure("a fresh exact request succeeds after rollback",
               !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)) && owner.reset());
    }

    {
        ...
        VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
        acquireResidentTextureUploadDestination(state, owner);
        const auto allocation_failure = VulkanInstanceDetail::acquireTextureUploadSampleBinding(
            owner, makeTextureUploadSampleBindingRequest(state, owner), failAllocation);
        ensureTextureUploadSampleBindingCode(allocation_failure, VulkanTextureUploadSampleBindingAcquireCode::AllocationFailure);
        ensure("wrapper allocation failure retires the fully resolved owner without publication",
               !owner.hasTextureUploadSampleBindingGeneration() && state.mDestroyTextureSampleSamplerCalls == 1 &&
                   state.mDestroyTextureSampleDescriptorSetLayoutCalls == 1 && state.mDestroyTextureSamplePipelineLayoutCalls == 1 &&
                   state.mDestroyTextureSampleDescriptorPoolCalls == 1);
        ensure("allocation rollback leaves the resident destination reusable",
               !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)) && owner.reset());
    }
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<145>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ...
    const std::uint64_t   resident_identity = owner.textureUploadSampleBindingResidentContentIdentity();

    ensure("completed transfer and source retirement preserve the destination-only sampled binding",
           owner.resetTextureUploadTransferGeneration() && owner.resetTextureUploadSourceGeneration() &&
               !owner.hasTextureUploadTransferGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && textureUploadSampleBindingMatches(state, owner));

    acquireCompleteSwapchainChildren(state, owner);
    ensureSwapchainChainRebuildOutcome(owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1280, 720 })),
                                       VulkanSwapchainChainRebuildOutcome::Ready);
    ensure("successful changed-extent rebuild preserves the exact device-scoped sampled binding",
           owner.textureUploadSampleBindingSampler() == sampler && owner.textureUploadSampleBindingDescriptorSet() == descriptor_set &&
               owner.textureUploadSampleBindingResidentContentIdentity() == resident_identity &&
               state.mDestroyTextureSampleSamplerCalls == 0);

    state.mSwapchainCreateResult = VK_ERROR_OUT_OF_HOST_MEMORY;
    const auto  failed_rebuild   = owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1440, 810 }));
    const auto& failure          = requireSwapchainChainRebuildError(failed_rebuild);
    ensure("failed changed-extent rebuild preserves the sampled binding exactly",
           failure.mCode == VulkanSwapchainChainRebuildCode::ChildFailure &&
               failure.mPhase == VulkanSwapchainChainRebuildPhase::Swapchain && owner.textureUploadSampleBindingSampler() == sampler &&
               owner.textureUploadSampleBindingDescriptorSet() == descriptor_set && state.mDestroyTextureSampleSamplerCalls == 0);

    state.mSwapchainCreateResult = VK_SUCCESS;
    ensureSwapchainChainRebuildOutcome(owner.rebuildSwapchainChain(makeSwapchainChainRebuildRequest(state, owner, VkExtent2D{ 1600, 900 })),
                                       VulkanSwapchainChainRebuildOutcome::Ready);

    ensure("direct binding reset preserves the complete rebuilt swapchain and resident destination",
           owner.resetTextureUploadSampleBindingGeneration() && !owner.hasTextureUploadSampleBindingGeneration() &&
               owner.hasSwapchainConfigurationGeneration() && owner.hasSwapchainGeneration() && owner.hasSwapchainImagesGeneration() &&
               owner.hasSwapchainPresentationTargetGeneration() && owner.hasSwapchainPresentationPipelineGeneration() &&
               owner.hasSwapchainReadbackGeneration() && owner.hasSwapchainFrameSlotGeneration() &&
               owner.hasTextureUploadDestinationGeneration() && owner.textureUploadDestinationIsResident() && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<146>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    ...
    ensure("destination reset retires the sampled binding, completed transfer, source, and destination",
           owner.resetTextureUploadDestinationGeneration() && !owner.hasTextureUploadSampleBindingGeneration() &&
               !owner.hasTextureUploadTransferGeneration() && !owner.hasTextureUploadSourceGeneration() &&
               !owner.hasTextureUploadDestinationGeneration() && owner.hasLogicalDeviceGeneration());
    const auto binding_pool     = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSampleDescriptorPool);
    const auto transfer_fence   = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyUploadTransferFence);
    const auto source_buffer    = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureUploadSourceBuffer);
    const auto destination_view = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    ensure("the destination cascade retires binding before transfer, source, and image dependencies",
           binding_pool != state.mEvents.end() && transfer_fence != state.mEvents.end() && source_buffer != state.mEvents.end() &&
               destination_view != state.mEvents.end() && binding_pool < transfer_fence && transfer_fence < source_buffer &&
               source_buffer < destination_view && state.mDestroyTextureSampleSamplerCalls == 1 &&
               state.mDestroyTextureSampleDescriptorPoolCalls == 1 && state.mDestroyTextureImageViewCalls == 1 && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<147>()
{
    FakeState                state;
    ScopedFakeState          scope(state);
    VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
    acquireTextureUploadSampleBindingChain(state, owner);
    state.mTextureUploadSampleBindingTeardownOwner = &owner;

    ensure("direct sampled-binding reset detaches before all four native destroy callbacks",
           owner.resetTextureUploadSampleBindingGeneration() && !owner.hasTextureUploadSampleBindingGeneration() &&
               state.mTextureUploadSampleBindingTeardownObservations.size() == 4);
    for (const auto& observation : state.mTextureUploadSampleBindingTeardownObservations)
    {
        ensure("binding teardown callbacks see detached accessors and reject every conflicting aggregate transaction",
               observation.mBindingDetached && observation.mDestinationLive && observation.mSourceLive && observation.mTransferLive &&
                   observation.mMoveProducedEmpty && !observation.mBindingResetSucceeded && !observation.mDestinationResetSucceeded &&
                   !observation.mLogicalResetSucceeded && !observation.mFullResetSucceeded && observation.mBindingAcquireError &&
                   observation.mBindingAcquireError->mCode == VulkanTextureUploadSampleBindingAcquireCode::NativeTeardownInProgress &&
                   observation.mDestinationAcquireError &&
                   observation.mDestinationAcquireError->mCode == VulkanTextureUploadDestinationAcquireCode::NativeTeardownInProgress &&
                   observation.mSourceAcquireError &&
                   observation.mSourceAcquireError->mCode == VulkanTextureUploadSourceAcquireCode::NativeTeardownInProgress &&
                   observation.mTransferAcquireError &&
                   observation.mTransferAcquireError->mCode == VulkanTextureUploadTransferAcquireCode::NativeTeardownInProgress &&
                   observation.mConfigurationAcquireError &&
                   observation.mConfigurationAcquireError->mCode == VulkanSwapchainConfigurationAcquireCode::NativeTeardownInProgress &&
                   observation.mRebuildError &&
                   observation.mRebuildError->mCode == VulkanSwapchainChainRebuildCode::NativeAcquisitionInProgress &&
                   observation.mRebuildError->mPhase == VulkanSwapchainChainRebuildPhase::Preflight);
    }

    state.mTextureUploadSampleBindingTeardownOwner = nullptr;
    ensure("the binding reacquires after guarded teardown",
           !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)));
    acquireCompleteSwapchainChildren(state, owner);
    state.mDeviceDestroyOwner  = &owner;
    state.mSurfaceDestroyOwner = &owner;
    state.mEvents.clear();
    ensure("logical reset retires the swapchain chain, then binding and destination, before the device",
           owner.resetLogicalDeviceGeneration());
    const auto swapchain        = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroySwapchain);
    const auto binding_pool     = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureSampleDescriptorPool);
    const auto destination_view = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyTextureImageView);
    const auto device           = std::find(state.mEvents.begin(), state.mEvents.end(), Event::DestroyDevice);
    ensure("logical teardown preserves the established swapchain-first dependency order",
           swapchain != state.mEvents.end() && binding_pool != state.mEvents.end() && destination_view != state.mEvents.end() &&
               device != state.mEvents.end() && swapchain < binding_pool && binding_pool < destination_view && destination_view < device &&
               state.mDeviceDestroyObservationMade && !state.mObservedTextureUploadSampleBindingAtDeviceDestroy &&
               !state.mObservedTextureUploadDestinationAtDeviceDestroy && owner.reset());
}

template<>
template<>
void render_vulkan_instance_test_object::test<148>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
        acquireResidentTextureUploadDestination(state, owner);
        TextureSampleBindingNestedContext             context{ &owner };
        const VulkanTextureUploadSampleBindingRequest request{ owner.nativeWindowGeneration(),
                                                               vulkanTextureUploadDestinationDescription(),
                                                               vulkanTextureUploadSampleBindingDescription(),
                                                               { &context, textureSampleBindingNestedOwnerIsCurrent },
                                                               { &context, textureSampleBindingNestedWindowIsCurrent } };
        const auto                                    outer = owner.acquireTextureUploadSampleBindingGeneration(request);
        ensureTextureUploadSampleBindingCode(outer, VulkanTextureUploadSampleBindingAcquireCode::TextureUploadSampleBindingAlreadyOwned);
        ensure("a nested exact acquisition wins while the stale outer occurrence rolls back without overwrite",
               context.mNestedAttempted && context.mNestedPublished && textureUploadSampleBindingMatches(state, owner) &&
                   state.mCreateTextureSampleSamplerCalls == 2 && state.mDestroyTextureSampleSamplerCalls == 1 &&
                   state.mDestroyTextureSampleDescriptorSetLayoutCalls == 1 && state.mDestroyTextureSamplePipelineLayoutCalls == 1 &&
                   state.mDestroyTextureSampleDescriptorPoolCalls == 1 && owner.reset());
        ensure("the winning same-bit occurrence owns a separate final teardown obligation",
               state.mDestroyTextureSampleSamplerCalls == 2 && state.mDestroyTextureSampleDescriptorSetLayoutCalls == 2 &&
                   state.mDestroyTextureSamplePipelineLayoutCalls == 2 && state.mDestroyTextureSampleDescriptorPoolCalls == 2);
    }

    {
        ...
        VulkanInstanceGeneration owner = takeGeneration(acquireVulkanInstanceGeneration(makeRequest(state)));
        acquireResidentTextureUploadDestination(state, owner);
        TextureSampleBindingDestinationAbaContext     context{ &state, &owner };
        const VulkanTextureUploadSampleBindingRequest request{ owner.nativeWindowGeneration(),
                                                               vulkanTextureUploadDestinationDescription(),
                                                               vulkanTextureUploadSampleBindingDescription(),
                                                               { &context, textureSampleBindingDestinationAbaOwnerIsCurrent },
                                                               { &context, textureSampleBindingDestinationAbaWindowIsCurrent } };
        const auto                                    stale = owner.acquireTextureUploadSampleBindingGeneration(request);
        ensureTextureUploadSampleBindingCode(stale, VulkanTextureUploadSampleBindingAcquireCode::TextureUploadDestinationNotLive);
        ensure("same-looking destination ABA replacement is rejected by epoch before binding-native work",
               context.mReset && context.mReacquiredResident && owner.hasTextureUploadDestinationGeneration() &&
                   owner.textureUploadDestinationIsResident() && !owner.hasTextureUploadSampleBindingGeneration() &&
                   state.mCreateTextureSampleSamplerCalls == 0);
        ensure("the replacement resident destination accepts a fresh binding transaction",
               !owner.acquireTextureUploadSampleBindingGeneration(makeTextureUploadSampleBindingRequest(state, owner)) && owner.reset());
    }
}
...
template<>
template<>
void render_vulkan_instance_test_object::test<149>()
{
    {
        FakeState                state;
        ScopedFakeState          scope(state);
        ...
        TextureSampleBindingRequestMutationContext context{ &owner, nullptr, true };
        VulkanTextureUploadSampleBindingRequest    request{ owner.nativeWindowGeneration(),
                                                         vulkanTextureUploadDestinationDescription(),
                                                         vulkanTextureUploadSampleBindingDescription(),
                                                            { &context, textureSampleBindingRequestMutationOwnerIsCurrent },
                                                            { &context, textureSampleBindingRequestMutationWindowIsCurrent } };
        context.mRequest  = &request;
        const auto result = owner.acquireTextureUploadSampleBindingGeneration(request);
        ensureTextureUploadSampleBindingCode(result, VulkanTextureUploadSampleBindingAcquireCode::StaleInstanceOwner);
        ensure("owner-callback shape mutation is rejected before dispatch through the immutable request snapshot",
               context.mMutated && context.mOwnerChecks == 1 && context.mWindowChecks == 0 && state.mCreateTextureSampleSamplerCalls == 0 &&
...

    {
        ...
        TextureSampleBindingRequestMutationContext context{ &owner };
        VulkanTextureUploadSampleBindingRequest    request{ owner.nativeWindowGeneration(),
                                                         vulkanTextureUploadDestinationDescription(),
                                                         vulkanTextureUploadSampleBindingDescription(),
                                                            { &context, textureSampleBindingRequestMutationOwnerIsCurrent },
        ...
        context.mRequest  = &request;
        const auto result = owner.acquireTextureUploadSampleBindingGeneration(request);
        ensureTextureUploadSampleBindingCode(result, VulkanTextureUploadSampleBindingAcquireCode::StaleWindowGeneration);
